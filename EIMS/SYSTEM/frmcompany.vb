...
Public Class frmcompany
    ...
    Private Sub clr()
        Me.Text = "Institute/Company Information Screen . . ."
        data_retrieve(1)
    End Sub

    Private Sub txtmembr_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtzip.Enter, txtcompany.Enter, txtcont2.Enter, txtcont1.Enter, txtaddress.Enter, txtmailid.Enter, txtdescri.Enter, txtwebsite.Enter, txtpernb.Enter, txtjuris.Enter
        sender.forecolor = Color.Blue
        sender.backcolor = Color.LavenderBlush

    End Sub

    Private Sub txtmembr_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtzip.Leave, txtcompany.Leave, txtcont2.Leave, txtcont1.Leave, txtaddress.Leave, txtmailid.Leave, txtdescri.Leave, txtwebsite.Leave, txtpernb.Leave, txtjuris.Leave
    ...
    Private Sub user_save()
        ...
        If cnfr = vbYes Then
            ...
            If ds.Tables(0).Rows.Count <> 0 Then
                SQLInsert("UPDATE company SET descri='" & Trim(txtdescri.Text) & "',co_snm='" & Trim(txtsnm.Text) & "',co_add1 ='" & _
                          ...
                          Trim(txtjuris.Text) & "', per_nb= '" & Trim(txtpernb.Text) & "', st_date='" & _
                          Format(start_date.Value, "dd/MMM/yyyy") & "', ed_date   ='" & _
                          Format(end_date.Value, "dd/MMM/yyyy") & "',co_img='' WHERE co_sl =" & Val(txtscrl.Text) & "")
    ...
    Private Sub data_retrieve(ByVal slno As Integer)
        Dim dsedit As DataSet = get_dataset("SELECT co_add1, zip_cd, co_ph1, co_ph2, co_mail, web, descri, st_date, ed_date, juri, per_nb, DataLength(co_img) AS co_img, co_nm,co_snm FROM company WHERE co_sl=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = slno
            txtcompany.Text = dsedit.Tables(0).Rows(0).Item("co_nm")
            txtsnm.Text = dsedit.Tables(0).Rows(0).Item("co_snm")
