...
Public Class frmpurchret
    Dim comd As String = "V"
    Private allowCoolMove As Boolean = False
    Private myCoolPoint As New Point
#Region "Start Up"
    Private Sub frmpurchret_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        cnfr = MessageBox.Show("Are You Sure To Close The Screen", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
        If cnfr = vbYes Then
            e.Cancel = False
            MDI.mnupret.Enabled = True
        Else
            e.Cancel = True
        End If
    End Sub

    Private Sub frmpurchret_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmpurchret_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        MDI.mnupret.Enabled = False
        usrprmsn("mnupret", cmnuadd, cmnuedit, cmenudel, cmenuview)
        comd = swapprmsn("mnupret", comd)
        Me.clr()
    ...
    Private Sub frmpurchret_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub clr()
        ...
        If comd = "E" Then
            ...
            Dim ds As DataSet = get_dataset("SELECT max(pret_no) as 'Max' FROM pret1")
            txtscroll.Text = 1
            cmbparty.Focus()
        ...
        ElseIf comd = "D" Then
            ...
            btnsave.Text = "Delete"
        Else
            Me.Text = "Purchase Return View Screen...."
            txtscroll.Text = ""
            btnsave.Text = "View"
        End If
        Me.clr1()
...
#Region "Key Press"
    ...
    Private Sub cmbpurtype_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles cmbpurtype.KeyPress
        key(cmbparty, e)
        SPKey(e)
    End Sub
...
#Region "Data save"
    Private Sub purchase_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO pret1 (pret_no,pret_dt,prt_code,pret_type,chal_no,chal_dt,ref_no,ref_dt,way_bno,trk_no,pret_amt,adj_amt," & _
                          "way_chr,dis_amt,chr_amt,nb,lr_no,lr_dt,usedby,godsl,rec_lock,usr_ent,ent_date,usr_mod,mod_date)" & _
                         "VALUES (" & Val(txtscroll.Text) & ",'" & Format(recdt.Value, "dd/MMM/yyyy") & "'," & Val(txtprtcd.Text) & ",2,'" & _
                         ...
                         Val(txtgrtotal.Text) & "," & Val(txtadj.Text) & ",'" & Trim(txtwaypfx.Text) & "'," & Val(txtdiscamt.Text) & "," & _
                          Val(txtchargamt.Text) & ",'" & Trim(txtnb.Text) & "','" & Trim(txtlrno.Text) & "','" & _
                          Format(lrdt.Value, "dd/MMM/yyyy") & "','N'," & Val(txtgodcd.Text) & ",'N'," & Val(usr_sl) & ",'" & _
        ...
        ElseIf comd = "M" Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT pret_no FROM pret1 WHERE pret_no=" & Val(txtscroll.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE pret1 SET pret_dt='" & Format(recdt.Value, "dd/MMM/yyyy") & "',prt_code=" & Val(txtprtcd.Text) & ",pret_type=2,chal_no='" & _
                         Trim(txtchno.Text) & "',chal_dt='" & Format(chdt.Value, "dd/MMM/yyyy") & "',ref_no='" & Trim(txtinvno.Text) & "',ref_dt='" & _
                         Format(invdt.Value, "dd/MMM/yyyy") & "',way_bno=" & Val(txtwbno.Text) & ",trk_no='" & Trim(txtvehno.Text) & "',pret_amt=" & _
                         Val(txtgrtotal.Text) & ",adj_amt=" & Val(txtadj.Text) & ",way_chr='" & Trim(txtwaypfx.Text) & "',dis_amt=" & _
                         Val(txtdiscamt.Text) & ",chr_amt=" & Val(txtchargamt.Text) & ",nb='" & Trim(txtnb.Text) & "',lr_no='" & _
                         Trim(txtlrno.Text) & "',lr_dt='" & Format(lrdt.Value, "dd/MMM/yyyy") & "',usr_mod=" & Val(usr_sl) & ",mod_date='" & _
                         Format(Now, "dd/MMM/yyyy HH:mm:ss") & "',usedby='N',rec_lock='N',godsl=" & _
                         Val(txtgodcd.Text) & " WHERE pret_no =" & Val(txtscroll.Text) & "")
                    Me.dv_save()
    ...
    Private Sub dv_save()
        If dv1.RowCount <> 0 Then
            ...
            For i As Integer = 0 To dv1.RowCount - 1
                ...
                Dim ds1 As DataSet = get_dataset("SELECT max(pret_sl) as 'Max' FROM pret2")
                mxno = 1
                If ds1.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO pret2(pret_sl,pret_no,pret_dt,prt_code,it_cd,pur_rt,pret_qty,it_amt,tto_amt,tax_cd,oth_amt,disc_amt,to_amt," & _
                          "stax,entax,mrp,ex_mrp,prd_tp,tr_tp,unt,free_qty,mul_rt,disc_rt1,disc_rt2,disc_amt1,disc_amt2,mrp_amt,bit_type) VALUES (" & _
                          Val(mxno) & "," & Val(txtscroll.Text) & ",'" & Format(recdt.Value, "dd/MMM/yyyy") & "'," & _
                          Val(txtprtcd.Text) & "," & Val(itcd) & "," & it_rt & "," & qty & "," & amt & "," & tto & "," & taxcd & "," & _
                          achrg & "," & adisc & "," & total & "," & vatamt & ",0," & mrp & "," & exmrp & ",'" & vb.Left(prdtp, 1) & "','" & _
                ...
                stock_OUT(itqt, itcd, vb.Left(prdtp, 1), vb.Left(trtp, 1))
            Next
        End If
    End Sub
...
#Region "Data Retrieve"
    ...
    Private Sub dv_disp()
        Dim dsedit As DataSet = get_dataset("SELECT pret1.pret_no AS 'Scrl', CONVERT(VARCHAR,pret1.pret_dt,103) AS 'Date',pret1.pret_no AS 'Ret.No', pret1.pret_dt AS 'Ret.Date', (CASE WHEN pret1.pret_type='1' THEN 'Cash' WHEN pret1.pret_type='2' THEN 'Credit' END) AS 'Type', party.pname AS 'Name Of the Party', STR(pret1.pret_amt,10,2) AS 'Amount' FROM pret1 LEFT OUTER JOIN party ON pret1.prt_code = party.prt_code ORDER BY pret1.pret_dt DESC, pret1.pret_no DESC")
        dv.Columns.Clear()
    ...
    Private Sub dv_view()
        Dim ds As DataSet = get_dataset("SELECT item.it_name, pret2.*, taxper.txname FROM pret2 LEFT OUTER JOIN taxper ON pret2.tax_cd = taxper.taxcd LEFT OUTER JOIN item ON pret2.it_cd = item.it_cd WHERE pret2.pret_no=" & Val(txtscroll.Text) & "")
        rw = 0
        dv1.Rows.Clear()
        If ds.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                dv1.Rows(rw).Cells(5).Value = Format(ds.Tables(0).Rows(i).Item("pret_qty"), "####0.00")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT party.pname,party.add1,party.cl_amt, pret1.*, godown.godnm FROM pret1 LEFT OUTER JOIN godown ON pret1.godsl = godown.godsl LEFT OUTER JOIN party ON pret1.prt_code = party.prt_code WHERE(pret1.pret_no = " & slno & ")")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscroll.Text = slno
            recdt.Value = dsedit.Tables(0).Rows(0).Item("pret_dt")
            ' txtorder.Text = dsedit.Tables(0).Rows(0).Item("po_no")
            txtinvno.Text = dsedit.Tables(0).Rows(0).Item("pret_no")
            invdt.Value = dsedit.Tables(0).Rows(0).Item("pret_dt")
            ...
            txtgrtotal.Text = Format(dsedit.Tables(0).Rows(0).Item("pret_amt"), "######0.00")
    ...
    Private Sub trans_view()
        ...
        Start1()
        Dim ds As DataSet
        ds = get_dataset("SELECT TOP (3) convert(varchar,pret_dt,3) 'Date',STR(mrp,10,2) 'MRP', STR(pur_rt,10,2) 'Rate',unt AS 'Unit' FROM pret2 WHERE (it_cd = " & Val(txtitemcd.Text) & ") AND (prt_code = " & Val(txtprtcd.Text) & ") ORDER BY pret_dt DESC")
        If ds.Tables(0).Rows.Count <> 0 Then
...
#End Region
    ...
    Private Sub cmbfrom_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub del1(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT pret_no,prt_code,pret_amt FROM pret1 WHERE pret_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            ...
            SQLInsert("DELETE FROM pret1 WHERE pret_no=" & Val(code) & "")
        End If
    End Sub

    'Delete data from Table2/Secondary Table
    Private Sub del2(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT pret_sl,pret_qty,it_cd,free_qty,mul_rt,prd_tp,tr_tp  FROM pret2 WHERE pret_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To dsdel.Tables(0).Rows.Count - 1
                ...
                stock_OUTDEL(itqt, itcd, prdtp, trtp)
                SQLInsert("DELETE FROM pret2 WHERE pret_sl=" & Val(pursl) & "")
            Next
        End If
    End Sub
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        ...
        If (prtnm Is Nothing OrElse prtnm = "") Then
            'User hit cancel
        Else
            Dim dssear As DataSet = get_dataset("SELECT pret1.pret_no AS 'Scrl', CONVERT(VARCHAR,pret1.pret_dt,103) AS 'Date',pret1.pret_no AS 'Ret.No', pret1.pret_dt AS 'Ret.Date', (CASE WHEN pret1.pret_type='1' THEN 'Cash' WHEN pret1.pret_type='2' THEN 'Credit' END) AS 'Type', party.pname AS 'Name Of the Party', STR(pret1.pret_amt,10,2) AS 'Amount' FROM pret1 LEFT OUTER JOIN party ON pret1.prt_code = party.prt_code WHERE (party.pname LIKE'" & prtnm & "%') ORDER BY party.pname")
            dv.Columns.Clear()
