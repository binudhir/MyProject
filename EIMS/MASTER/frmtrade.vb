...
Public Class frmtrade
    ...
    Private Sub frmtrade_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub frmtrade_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmtrade_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub frmtrade_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub clr()
        Me.Text = "Branch Master Entry Screen . . ."
        txtscrl.Text = ""
        txttradnm.Text = ""
        cmblock.SelectedIndex = 0
        Dim ds As DataSet = get_dataset("SELECT max(trad_cd) as 'Max' FROM trad")
        txtscrl.Text = 1
    ...
    Private Sub sender_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txttradnm.Enter, cmblock.Enter
        sender.forecolor = Color.Blue
        sender.backcolor = Color.LavenderBlush

    End Sub

    Private Sub sender_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txttradnm.Leave, cmblock.Leave
    ...
    Private Sub txttradnm_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txttradnm.KeyPress
        key(cmblock, e)
        SPKey(e)
    End Sub
    ...
    Private Sub trad_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO trad (trad_cd,trad_nm,rec_lock) VALUES (" & Val(txtscrl.Text) & ",'" & _
                          Trim(txttradnm.Text) & "','" & _
        ...
        Else
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT trad_cd FROM trad WHERE trad_cd=" & Val(txtscrl.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE trad SET trad_nm='" & Trim(txttradnm.Text) & "',rec_lock='" & _
                              vb.Left(cmblock.Text, 1) & "' WHERE trad_cd =" & Val(txtscrl.Text) & "")
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY trad_nm) AS 'Sl.',  trad_nm AS 'Branch Name', trad_cd,rec_lock FROM trad ORDER BY trad_nm")
        dv.Columns.Clear()
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT * FROM trad WHERE trad_cd=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = slno
            txttradnm.Text = dsedit.Tables(0).Rows(0).Item("trad_nm")
    ...
    Private Sub cmnuedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuedit.Click
        If dv.RowCount <> 0 Then
            comd = "M"
            btnsave.Text = "Modify"
            Me.Text = "Branch Master Modification Screen . . ."
    ...
    Private Sub cmenudel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenudel.Click
        ...
        If dv.RowCount <> 0 Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT trad_cd FROM trad WHERE trad_cd=" & Val(slno) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    'End If
                    If fnd = 0 Then
                        SQLInsert("DELETE FROM trad WHERE trad_cd =" & Val(slno) & "")
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            ...
            Me.Text = "Branch Master View Screen . . ."
            slno = dv.SelectedRows(0).Cells(2).Value
            Me.dv_edit(slno)
            dv.Visible = False
            txttradnm.Focus()
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        authnm = InputBox("Enter The Branch Name.", "Search Box...", Nothing)
        If (authnm Is Nothing OrElse authnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY trad_nm) AS 'Sl.',  trad_nm AS 'Branch Name', trad_cd,rec_lock FROM trad WHERE (trad.trad_nm LIKE'" & authnm & "%') ORDER BY trad_nm")
            dv.Columns.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                dv.DataSource = dsedit.Tables(0)
                dv.Columns(2).Visible = False
