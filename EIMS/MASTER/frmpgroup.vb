...
Public Class frmpgroup
    ...
    Private Sub frmpgroup_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        ...
        If cnfr = vbYes Then
            e.Cancel = False
            MDI.mnuPeriodGroup.Enabled = True
        Else
            e.Cancel = True
        End If
    End Sub
    ...
    Private Sub frmpgroup_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        MDI.mnuPeriodGroup.Enabled = False
        usrprmsn("mnuCountry", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub

    Private Sub frmpgroup_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        ...
        If (authnm Is Nothing OrElse authnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT * FROM pgroup WHERE (pgroup.pgrp_nm LIKE'" & authnm & "%')  ORDER BY pgrp_cd")
            dv.Rows.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                Dim rw As Integer = 0
                For i As Integer = 0 To dsedit.Tables(0).Rows.Count - 1
