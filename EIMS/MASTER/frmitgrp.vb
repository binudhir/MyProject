...
Public Class frmitgrp
    ...
    Private Sub frmitgrp_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuProductgrp", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub clr()
        ...
        Dim ds As DataSet = get_dataset("SELECT max(grp_cd) as 'Max' FROM item_group")
        txtscrl.Text = 1
    ...
    Private Sub trad_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO item_group (grp_cd,grp_nm,rec_lock) VALUES (" & Val(txtscrl.Text) & ",'" & _
        ...
        Else
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT grp_cd FROM item_group WHERE grp_cd=" & Val(txtscrl.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE item_group SET grp_nm='" & Trim(txtuntnm.Text) & "',rec_lock='" & _
                              vb.Left(cmblock.Text, 1) & "' WHERE grp_cd =" & Val(txtscrl.Text) & "")
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY grp_cd) AS 'Sl.', grp_cd, grp_nm AS 'Group Name', rec_lock FROM item_group ORDER BY grp_cd")
        dv.Columns.Clear()
        If dsedit.Tables(0).Rows.Count <> 0 Then
            ...
            '    dv.Rows(i).Cells(1).Value = dsedit.Tables(0).Rows(rw).Item("grp_nm")
            '    dv.Rows(i).Cells(2).Value = dsedit.Tables(0).Rows(rw).Item("grp_cd")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT * FROM item_group WHERE grp_cd=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = slno
            txtuntnm.Text = dsedit.Tables(0).Rows(0).Item("grp_nm")
    ...
    Private Sub cmenudel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenudel.Click
        ...
        If dv.RowCount <> 0 Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT grp_cd FROM item_group WHERE grp_cd=" & Val(slno) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    'End If
                    If fnd = 0 Then
                        SQLInsert("DELETE FROM item_group WHERE grp_cd =" & Val(slno) & "")
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            ...
            Me.Text = "Product Group Master View Screen . . ."
            slno = dv.SelectedRows(0).Cells(0).Value
            Me.dv_edit(slno)
            dv.Visible = False
            txtuntnm.Focus()
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        authnm = InputBox("Enter The Product Group Name.", "Search Box...", Nothing)
        If (authnm Is Nothing OrElse authnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY grp_cd) AS 'Sl.', grp_cd, grp_nm AS 'Group Name', rec_lock FROM item_group WHERE (item_group.grp_nm LIKE'" & authnm & "%') ORDER BY grp_cd")
            dv.Columns.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                dv.DataSource = dsedit.Tables(0)
                dv.Columns(1).Visible = False
