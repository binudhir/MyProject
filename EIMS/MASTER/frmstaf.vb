...
Public Class frmstaf
    ...
    Private Sub frmstaf_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuEmployee", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub txtmail_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmail.Enter
        ...
        txtmail.Font = New System.Drawing.Font("Verdana", 8, FontStyle.Bold)
    End Sub

    Private Sub txtmail_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmail.Leave
        txtmail.ForeColor = Color.Black
        txtmail.BackColor = Color.White
        txtmail.Font = New System.Drawing.Font("Verdana", 8, FontStyle.Regular)
    End Sub
...
#End Region
    ...
    Private Sub btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.Click
        If Trim(txtpreid.Text) = "" And comd = "E" Then
        ...
        If Trim(cmbsub.Text) = "" Or Val(txtsubcd.Text) = 0 Then
            MessageBox.Show("The Subject Name Should Not Be Blank", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            TabControl1.SelectedIndex = 1
            cmbsub.Focus()
            Exit Sub
        End If
        ...
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                SQLInsert("INSERT INTO staf(staf_sl,nm_pri,staf_nm,short_nm,device_code,emp_tp,desg_cd,dept_cd,doj,dol,per_add1," & _
                          ...
                          "rec_lock,moth_nm,pf_req,pf_reg,esi_req,esi_reg,dist_cd1,dist_cd2,mail_id,login_req,pswd_crtd,sub_cd," & _
                          "cont_no1,grd_mob,last_emp,tot_exp,basic_pay,leaved,dom,room_req,conync_req,cantn_req,sms_req,mail_req,id_pfx,stafid_no,staf_id) VALUES (" & _
                          Val(txtscrl.Text) & ",'" & Trim(cmbprinm.Text) & "','" & Trim(txtempnm.Text) & "','" & Trim(txtsrtnm.Text) & "'," & _
                          Val(txtdevcd.Text) & ",'" & vb.Left(cmbposttype.Text, 1) & "'," & Val(txtdesgcd.Text) & "," & Val(txtdeptcd.Text) & ",'" & _
                          Format(regdt.Value, "dd/MMM/yyyy") & "','" & Format(regdt.Value, "dd/MMM/yyyy") & "','" & Trim(txtadd1.Text) & "','" & _
                          Trim(txtpin1.Text) & "','" & Trim(txtph1.Text) & "','" & Trim(txtadd2.Text) & "','" & Trim(txtpin2.Text) & "','" & _
                          Format(regdt.Value, "dd/MMM/yyyy") & "','" & IIf(rdmale.Checked, "M", "F") & "','" & IIf(chkmard.Checked, "Y", "N") & "','" & _
                          Trim(txtnatlty.Text) & "','" & Trim(txtrelgon.Text) & "','','" & vb.Left(cmbnmof.Text, 1) & "','" & Trim(txtnmof.Text) & "','','" & _
                          vb.Left(cmblock.Text, 1) & "','" & Trim(txtmotnm.Text) & "','" & _
                          IIf(chkpf.Checked, "Y", "N") & "','" & Trim(txtpfredg.Text) & "','" & IIf(chkesi.Checked, "Y", "N") & "','" & _
                          Trim(txtesino.Text) & "'," & Val(txtdistcd1.Text) & "," & Val(txtdistcd2.Text) & ",'" & Trim(txtmail.Text) & "','" & _
                          vb.Left(cmblogin.Text, 1) & "','N'," & Val(txtsubcd.Text) & ",'" & Trim(txtph2.Text) & "','" & _
                          Trim(txtgrdmob.Text) & "','" & Trim(txtlastemp.Text) & "'," & Val(txtexp.Text) & "," & Val(txtbasic.Text) & ",'N','" & _
                          Format(dom.Value, "dd/MMM/yyyy") & "','" & IIf(chkroom.Checked, "Y", "N") & "','" & IIf(chktrans.Checked, "Y", "N") & "','" & _
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE staf SET nm_pri='" & Trim(cmbprinm.Text) & "',staf_nm='" & Trim(txtempnm.Text) & "',short_nm='" & _
                              ...
                              Format(dob.Value, "dd/MMM/yyyy") & "',gndr='" & IIf(rdmale.Checked, "M", "F") & "',married='" & IIf(chkmard.Checked, "Y", "N") & "',nation='" & _
                              Trim(txtnatlty.Text) & "',religion='" & Trim(txtrelgon.Text) & "',f_g_h='" & vb.Left(cmbnmof.Text, 1) & "',fgh_nm='" & _
                              Trim(txtnmof.Text) & "',staf_image='',rec_lock='" & vb.Left(cmblock.Text, 1) & "',moth_nm='" & _
                              ...
                              Val(txtdistcd2.Text) & ",mail_id='" & Trim(txtmail.Text) & "',sub_cd=" & Val(txtsubcd.Text) & ",cont_no1='" & _
                              Trim(txtph2.Text) & "',grd_mob='" & Trim(txtgrdmob.Text) & "',last_emp='" & Trim(txtlastemp.Text) & "',tot_exp=" & _
                              Val(txtexp.Text) & ",basic_pay=" & Val(txtbasic.Text) & ",dom='" & Format(dom.Value, "dd/MMM/yyyy") & "',room_req='" & _
                              IIf(chkroom.Checked, "Y", "N") & "',conync_req='" & IIf(chktrans.Checked, "Y", "N") & "',cantn_req='" & _
                              IIf(chkcnteen.Checked, "Y", "N") & "',sms_req='" & IIf(chksms.Checked, "Y", "N") & "',mail_req='" & _
                              IIf(chkmail.Checked, "Y", "N") & "' WHERE staf_sl =" & Val(txtscrl.Text) & "")
                    Me.quali_save()
                    If txtphoto.Text = "Y" Then
                        Me.save_image()
                    End If
...
#Region "Data Display"
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT staf.staf_sl, staf.nm_pri, staf.staf_nm, staf.desg_cd, staf.dept_cd, staf.per_add1, staf.cont_no, dept.dept_nm, desg.desg_nm, staf.rec_lock, subject_mst.sub_nm FROM staf LEFT OUTER JOIN subject_mst ON staf.sub_cd = subject_mst.sub_cd LEFT OUTER JOIN desg ON staf.desg_cd = desg.desg_cd LEFT OUTER JOIN dept ON staf.dept_cd = dept.dept_cd ORDER BY staf.staf_nm")
        dv.Rows.Clear()
        If dsedit.Tables(0).Rows.Count <> 0 Then
            ...
            For i As Integer = 0 To dsedit.Tables(0).Rows.Count - 1
                ...
                dv.Rows(i).Cells(1).Value = dsedit.Tables(0).Rows(rw).Item("nm_pri") & " " & dsedit.Tables(0).Rows(rw).Item("staf_nm")
                dv.Rows(i).Cells(2).Value = dsedit.Tables(0).Rows(rw).Item("per_add1")
                dv.Rows(i).Cells(3).Value = dsedit.Tables(0).Rows(rw).Item("cont_no")
                dv.Rows(i).Cells(4).Value = dsedit.Tables(0).Rows(rw).Item("dept_nm")
                dv.Rows(i).Cells(5).Value = dsedit.Tables(0).Rows(rw).Item("desg_nm")
                dv.Rows(i).Cells(6).Value = dsedit.Tables(0).Rows(rw).Item("sub_nm")
                dv.Rows(i).Cells(7).Value = dsedit.Tables(0).Rows(rw).Item("staf_sl")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT dist.dist_nm, stat.stat_nm, staf.*, subject_mst.sub_nm, desg.desg_nm, dept.dept_nm, stat.stat_nm,DataLength(staf.staf_image) AS staf_img FROM staf LEFT OUTER JOIN dept ON staf.dept_cd = dept.dept_cd LEFT OUTER JOIN desg ON staf.desg_cd = desg.desg_cd LEFT OUTER JOIN subject_mst ON staf.sub_cd = subject_mst.sub_cd LEFT OUTER JOIN dist ON staf.dist_cd1 = dist.dist_cd LEFT OUTER JOIN stat ON dist.stat_cd = stat.stat_cd WHERE staf.staf_sl=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = slno
...
#End Region
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            ...
            Me.Text = "Employee Registration Master View Screen . . ."
            slno = dv.SelectedRows(0).Cells(7).Value
            Me.dv_edit(slno)
            dv.Visible = False
            txtempnm.Focus()
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        stafnm = InputBox("Enter The Employee Name.", "Search Box...", Nothing)
        If (stafnm Is Nothing OrElse stafnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT staf.staf_sl, staf.nm_pri, staf.staf_nm, staf.desg_cd, staf.dept_cd, staf.per_add1, staf.cont_no, dept.dept_nm, desg.desg_nm, staf.rec_lock, subject_mst.sub_nm FROM staf LEFT OUTER JOIN subject_mst ON staf.sub_cd = subject_mst.sub_cd LEFT OUTER JOIN desg ON staf.desg_cd = desg.desg_cd LEFT OUTER JOIN dept ON staf.dept_cd = dept.dept_cd WHERE (staf.staf_nm LIKE'" & stafnm & "%') ORDER BY staf.staf_nm")
            dv.Rows.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                Dim rw As Integer = 0
                For i As Integer = 0 To dsedit.Tables(0).Rows.Count - 1
