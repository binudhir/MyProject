...
Public Class frmquali
    ...
    Private Sub frmquali_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuQualification", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub trad_save()
        ...
        Else
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE education SET edu_nm='" & Trim(txttradnm.Text) & "',rec_lock='" & _
    ...
    Private Sub dv_disp()
        ...
        If dsedit.Tables(0).Rows.Count <> 0 Then
            ...
            For i As Integer = 0 To dv.Rows.Count - 1
                ...
                '    dv.Rows(i).Cells(1).Value = dsedit.Tables(0).Rows(rw).Item("edu_nm")
                '    dv.Rows(i).Cells(2).Value = dsedit.Tables(0).Rows(rw).Item("edu_cd")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        ...
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = slno
            txttradnm.Text = dsedit.Tables(0).Rows(0).Item("edu_nm")
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        authnm = InputBox("Enter The Qualification Name.", "Search Box...", Nothing)
        If (authnm Is Nothing OrElse authnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY edu_cd) AS 'Sl.', edu_nm AS 'Qualification', edu_cd, rec_lock FROM education WHERE (education.edu_nm LIKE'" & authnm & "%') ORDER BY edu_cd")
            dv.Columns.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                dv.DataSource = dsedit.Tables(0)
                dv.Columns(2).Visible = False
