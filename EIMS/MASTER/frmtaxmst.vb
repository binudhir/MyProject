...
Public Class frmtaxmst
    ...
    Private Sub frmtaxmst_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuTax", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub txtmembr_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtsrtnm.Enter, txttaxnm.Enter, cmblock.Enter, txttaxamt.Enter, cmbtype.Enter
        sender.forecolor = Color.Blue
        sender.backcolor = Color.LavenderBlush
    End Sub

    Private Sub txtmembr_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtsrtnm.Leave, txttaxnm.Leave, cmblock.Leave, txttaxamt.Leave, cmbtype.Leave
    ...
    Private Sub txtcustnm_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txttaxnm.KeyPress
        key(txtsrtnm, e)
        SPKey(e)
    End Sub
    ...
    Private Sub txtgrpnm_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txttaxnm.LostFocus
        ...
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO taxper (taxcd,taxnm,txname,taxcat,taxamt,rec_lock,taxsl,tax_tp,pay_tp,cr_alow) VALUES (" & Val(txtscrl.Text) & ",'" & _
                          Trim(txttaxnm.Text) & "','" & Trim(txtsrtnm.Text) & "'," & _
                          vb.Left(cmbtype.Text, 1) & "," & Val(txttaxamt.Text) & ",'" & _
                          vb.Left(cmblock.Text, 1) & "','','V','Y','N')")
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE taxper SET taxnm='" & Trim(txttaxnm.Text) & "',txname='" & Trim(txtsrtnm.Text) & "',taxcat='" & _
                          vb.Left(cmbtype.Text, 1) & "',taxamt=" & Val(txttaxamt.Text) & ",rec_lock='" & _
                              vb.Left(cmblock.Text, 1) & "' WHERE taxcd =" & Val(txtscrl.Text) & "")
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY taxnm) AS 'Sl.',taxnm AS 'Tax Name', (CASE WHEN taxcat='1' THEN 'VAT' WHEN taxcat='2' THEN 'MRP' END) AS 'Type', STR(taxamt,10,2) AS 'Tax(%)', taxcd, rec_lock FROM taxper ORDER BY taxnm")
        dv.Columns.Clear()
        If dsedit.Tables(0).Rows.Count <> 0 Then
            ...
            '        dv.Rows(i).Cells(1).Value = dsedit.Tables(0).Rows(rw).Item("taxnm")
            '        If dsedit.Tables(0).Rows(rw).Item("taxcat") = "1" Then
            '            dv.Rows(i).Cells(2).Value = "VAT"
            '        Else
            '            dv.Rows(i).Cells(2).Value = "MRP"
            '        End If
            '        dv.Rows(i).Cells(3).Value = Format(dsedit.Tables(0).Rows(rw).Item("taxamt"), "######0.00")
            '        dv.Rows(i).Cells(4).Value = dsedit.Tables(0).Rows(rw).Item("taxcd")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        ...
        If dsedit.Tables(0).Rows.Count <> 0 Then
            ...
            txttaxamt.Text = Format(dsedit.Tables(0).Rows(0).Item("taxamt"), "######0.00")
            cmbtype.SelectedIndex = Val(dsedit.Tables(0).Rows(0).Item("taxcat")) - 1
    ...
    Private Sub cmnuadd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuadd.Click
        ...
        comd = "E"
        btnsave.Text = "Add"
        Me.Text = "Tax Category Master Entry Screen . . ."
        dv.Visible = False
        txttaxnm.Focus()
    End Sub
    ...
    Private Sub cmnuedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuedit.Click
        If dv.RowCount <> 0 Then
            comd = "M"
            btnsave.Text = "Modify"
            Me.Text = "Tax Category Master Modification Screen . . ."
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            ...
            Me.Text = "Tax Category Master View Screen . . ."
            slno = dv.SelectedRows(0).Cells(4).Value
            Me.dv_edit(slno)
            dv.Visible = False
            txttaxnm.Focus()
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        authnm = InputBox("Enter The Tax Category Name.", "Search Box...", Nothing)
        If (authnm Is Nothing OrElse authnm = "") Then
            'User hit cancel
        Else
            Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY taxnm) AS 'Sl.',taxnm AS 'Tax Name', (CASE WHEN taxcat='1' THEN 'VAT' WHEN taxcat='2' THEN 'MRP' END) AS 'Type', STR(taxamt,10,2) AS 'Tax(%)', taxcd, rec_lock FROM taxper WHERE (taxper.taxnm LIKE'" & authnm & "%') ORDER BY taxnm")
            dv.Columns.Clear()
            If dsedit.Tables(0).Rows.Count <> 0 Then
                dv.DataSource = dsedit.Tables(0)
                dv.Columns(4).Visible = False
