...
Public Class rep_dstdlist
    Dim comd As String

    Private Sub rep_dstdlist_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_dstdlist_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_dstdlist_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Dock = DockStyle.Fill
        Me.clr()
    End Sub

    Private Sub clr()
        Me.Text = "Defaulter Student List. . ."
        hddr = Me.Text
        txtsesncd.Text = 0
        txtstrmcd.Text = 0
        txtacdncd.Text = 0
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        If chkabst.Checked = False Then
            ds = get_dataset("SELECT  ROW_NUMBER() OVER(ORDER BY " & orderby & ") AS 'Sl.', stud.reg_no AS 'Regd.No', stud.stud_nm AS 'Student Name', trad.trad_nm AS 'Stream', semester.sem_nm AS 'Class', stud.council_reg AS 'Council No', CONVERT(VARCHAR, stud.dob, 103) AS 'Date Of Birth', stud.fathr_nm AS 'Father Name', stud.pre_add1 AS 'Address', dist.dist_nm AS 'District', stat.stat_nm AS 'State', stud.pre_ph1 AS 'Stud Mob', stud.per_ph1 AS 'Father Mob', stud.email AS 'Email', stud.gndr AS 'M/F', (CASE WHEN stud_caste = 1 THEN 'Gen' WHEN stud_caste = 2 THEN 'ST' WHEN stud_caste = 3 THEN 'SC' WHEN stud_caste = 4 THEN 'OBC' END) AS 'Caste', (CASE WHEN stud.hostel_req = 'N' THEN 'No' WHEN stud.hostel_req = 'Y' THEN 'Yes' END) AS 'Hostel', STR(schd2.due_amt,10,2) AS 'Due Amount', CONVERT(varchar,schd2.due_dt,103) AS 'Due Date' FROM dist LEFT OUTER JOIN  stat ON dist.stat_cd = stat.stat_cd RIGHT OUTER JOIN  schd2 RIGHT OUTER JOIN  stud ON schd2.stud_sl = stud.stud_sl ON dist.dist_cd = stud.pre_dist_cd RIGHT OUTER JOIN  semester RIGHT OUTER JOIN  stud_hstry ON semester.sem_cd = stud_hstry.sem_cd LEFT OUTER JOIN  trad ON stud_hstry.trad_cd = trad.trad_cd ON stud.stud_sl = stud_hstry.stud_sl WHERE schd2.due_dt < GETDATE() AND (stud_hstry.sesn_cd = " & Val(txtsesncd.Text) & ")" & conditn & " ORDER BY " & orderby & "")
        Else
