...
Imports System.IO
Public Class rep_colregstud
    Dim comd As String
    Dim con_img As New SqlConnection(connectionstrng)
    Dim cmd_img As SqlCommand

    Private Sub rep_colregstud_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_colregstud_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_colregstud_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        Start1()
        Dim ds As DataSet
        ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY collrec1.coll_dt) AS 'Sl.', STR(collrec2.coll_no,10,0) AS 'Collection No.', CONVERT(varchar,collrec2.coll_dt,103) AS 'Collection Date', stud.stud_nm AS 'Student Name', semester.sem_nm AS 'Class', coll.coll_nm AS 'Collection Name', STR(collrec2.coll_amt,10,2) AS 'Collection Amount', STR(collrec1.tot_amt,10,2) AS 'Total' FROM  coll RIGHT OUTER JOIN  stud RIGHT OUTER JOIN collrec2 LEFT OUTER JOIN  semester ON collrec2.sem_cd = semester.sem_cd ON stud.stud_sl = collrec2.stud_sl ON coll.coll_cd = collrec2.coll_cd RIGHT OUTER JOIN  collrec1 ON collrec2.coll_no = collrec1.coll_no WHERE (stud.reg_no = '" & Trim(txtregdno.Text) & "') AND (collrec1.coll_dt>='" & Format(startdt.Value, "dd/MMM/yyyy") & "') AND (collrec1.coll_dt<='" & Format(enddt.Value, "dd/MMM/yyyy") & "') ORDER BY collrec1.coll_dt")
        If ds.Tables(0).Rows.Count <> 0 Then
