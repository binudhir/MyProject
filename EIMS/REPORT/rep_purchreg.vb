...
#End Region
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        Else
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY " & orderby & ") AS 'Sl.',  STR(purch1.pur_no,10,0) AS 'Pur No.', CONVERT(varchar,purch1.pur_dt,103) AS 'Purchase Date',  purch1.pur_type AS 'Purchase Type', party.pname AS 'Party Name', item.it_name AS 'Product', STR(purch2.pur_qty,10,2) AS 'Quantity', STR(purch2.free_qty,10,2) AS 'Free', item.unt1 AS 'Unit',  STR(purch2.pur_rt,10,2) AS 'Purchase Rate', STR(purch2.it_amt,10,2) AS 'Product Amount', STR(purch2.stax,10,2) AS 'TAX', STR(purch2.it_tot,10,2) AS 'Total', STR(purch1.chr_amt,10,2) AS 'Charge Amount', STR(purch1.dis_amt,10,2) AS 'Discount Amount', STR(purch1.adj_amt,10,2) AS 'Adjust Amount', STR(purch1.pur_amt,10,2) AS 'TOTAL AMOUNT' FROM  purch1 LEFT OUTER JOIN  purch2 LEFT OUTER JOIN party ON purch2.prt_code = party.prt_code LEFT OUTER JOIN  item ON purch2.it_cd = item.it_cd ON purch1.pur_no = purch2.pur_no  WHERE (purch1.pur_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (purch1.pur_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY " & orderby & "")
        End If
        If ds.Tables(0).Rows.Count <> 0 Then
            ...
            For i As Integer = 0 To dv.Rows.Count - 1
                If ds.Tables(0).Rows(i).Item("Purchase type") = "1" Then
                    ds.Tables(0).Rows(i).Item("Purchase type") = "Cash"
                Else
                    ds.Tables(0).Rows(i).Item("Purchase type") = "Credit"
                End If
                If slno = Val(dv.Rows(i).Cells(1).Value) Then
                    ...
                    'dv.Rows(i).Cells(18).Value = ""
                    'dv.Rows(i).Cells(19).Value = ""
                    'dv.Rows(i).Cells(20).Value = ""
                    'dv.Rows(i).Cells(21).Value = ""
                Else
