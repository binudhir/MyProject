...
Public Class rep_colregall
    Dim comd As String

    Private Sub rep_colregall_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_colregall_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_colregall_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        ds = get_dataset("SELECT   collrec1.coll_no ,collrec1.coll_dt, stud_hstry.reg_no ,stud.stud_nm ,sesion1.sesn_nm ,trad.trad_nm ,semester. sem_nm ,collrec1.pay_mod,collrec1.tot_amt,collrec1.bank_nm,collrec1.nb  FROM stud RIGHT OUTER JOIN stud_hstry ON stud.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN semester ON stud_hstry.sem_cd = semester.sem_cd RIGHT OUTER JOIN collrec1 ON stud_hstry.stud_sl = collrec1.stud_sl WHERE (stud_hstry.active = 'Y') AND (collrec1.coll_dt>='" & Format(startdt.Value, "dd/MMM/yyyy") & "') AND (collrec1.coll_dt<='" & Format(enddt.Value, "dd/MMM/yyyy") & "') ORDER BY collrec1.coll_dt  ")
        If ds.Tables(0).Rows.Count <> 0 Then
            MDI.pb.Maximum = ds.Tables(0).Rows.Count + 1
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                dv.Rows.Add()
                dv.Rows(rw).Cells(0).Value = rw + 1
                dv.Rows(rw).Cells(1).Value = ds.Tables(0).Rows(i).Item("coll_no") & " / Sc"
                ...
                dv.Rows(rw).Cells(9).Style.Alignment = DataGridViewContentAlignment.BottomRight
        ...
        ds1 = get_dataset("SELECT   ocollrec1.coll_no ,ocollrec1.coll_dt, stud_hstry.reg_no ,stud.stud_nm ,sesion1.sesn_nm ,trad.trad_nm ,semester. sem_nm ,ocollrec1.pay_mod,ocollrec1.tot_amt,ocollrec1.bank_nm,ocollrec1.nb  FROM stud RIGHT OUTER JOIN stud_hstry ON stud.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN semester ON stud_hstry.sem_cd = semester.sem_cd RIGHT OUTER JOIN ocollrec1 ON stud_hstry.stud_sl = ocollrec1.stud_sl WHERE (stud_hstry.active = 'Y') AND (ocollrec1.coll_dt>='" & Format(startdt.Value, "dd/MMM/yyyy") & "') AND (ocollrec1.coll_dt<='" & Format(enddt.Value, "dd/MMM/yyyy") & "') ORDER BY ocollrec1.coll_dt ")
        If ds1.Tables(0).Rows.Count <> 0 Then
            MDI.pb.Maximum = ds1.Tables(0).Rows.Count + 1
            For i As Integer = 0 To ds1.Tables(0).Rows.Count - 1
                dv.Rows.Add()
                dv.Rows(rw).Cells(0).Value = rw + 1
                dv.Rows(rw).Cells(1).Value = ds1.Tables(0).Rows(i).Item("coll_no") & " / Non-Sc"
                dv.Rows(rw).Cells(2).Value = Format(ds1.Tables(0).Rows(i).Item("coll_dt"), "dd/MM/yyyy")
                dv.Rows(rw).Cells(3).Value = ds1.Tables(0).Rows(i).Item("reg_no")
                dv.Rows(rw).Cells(4).Value = ds1.Tables(0).Rows(i).Item("stud_nm")
                dv.Rows(rw).Cells(5).Value = ds1.Tables(0).Rows(i).Item("sesn_nm")
                dv.Rows(rw).Cells(6).Value = ds1.Tables(0).Rows(i).Item("trad_nm")
                dv.Rows(rw).Cells(7).Value = ds1.Tables(0).Rows(i).Item("sem_nm")

                If ds1.Tables(0).Rows(i).Item("pay_mod") = "1" Then
                    dv.Rows(rw).Cells(8).Value = "Cash"
                    dv.Rows(rw).Cells(10).Value = ds1.Tables(0).Rows(i).Item("nb")
                Else
                    dv.Rows(rw).Cells(8).Value = "DD/Cheque"
                    dv.Rows(rw).Cells(10).Value = ds1.Tables(0).Rows(i).Item("bank_nm") & "," & ds1.Tables(0).Rows(i).Item("nb")
                End If
                dv.Rows(rw).Cells(9).Value = Format(ds1.Tables(0).Rows(i).Item("tot_amt"), "#########.00")
                dv.Rows(rw).Cells(9).Style.Alignment = DataGridViewContentAlignment.BottomRight
                tot_coll1 = tot_coll1 + Val(ds1.Tables(0).Rows(i).Item("tot_amt"))
                rw = rw + 1
                Call pbincr()
            Next
        End If
        If Val(tot_coll) <> 0 Or Val(tot_coll1) <> 0 Then
            dv.Rows.Add()
            ...
            dv.Rows(rw).Cells(9).Value = Format(Val(tot_coll) + Val(tot_coll1), "########0.00")
            dv.Rows(rw).Cells(9).Style.Alignment = DataGridViewContentAlignment.BottomRight
            ...
            'rw = rw + 1
        End If
        GroupBox3.Text = "Student Collection Register From" & startdt.Value & " To " & enddt.Value
        hddr = GroupBox3.Text
