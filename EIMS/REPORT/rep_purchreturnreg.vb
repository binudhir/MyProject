...
Public Class rep_purchreturnreg
    Dim comd As String

    Private Sub rep_purchreturnreg_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_purchreturnreg_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_purchreturnreg_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#End Region
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        If (chkdetail.Checked = True) Then
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY " & orderby & ") AS 'Sl.',  STR(pret1.pret_no,10,0) AS 'Purchase Return No.', CONVERT(varchar,pret1.pret_dt,103) AS 'Purchase Return Date', pret1.ref_no AS 'Reference No.', CONVERT(varchar,pret1.ref_dt,103) AS 'Reference Date', pret2.tr_tp AS 'Purchase Type', party.pname AS 'Party Name', item.it_name AS 'Product', STR(pret2.pret_qty,10,2) AS 'Quantity', STR(pret2.free_qty,10,2) AS 'Free', item.unt1 AS 'Unit', STR(pret2.mrp,10,2) AS 'M.R.P.', STR(pret2.pur_rt,10,2) AS 'Purchase Rate', STR(pret2.it_amt,10,2) AS 'Product Amount', STR(pret2.disc_amt1,10,2) AS 'Co. Dis. Amt.', STR(pret2.disc_amt2,10,2) AS 'Tr. Dis. Amt.',  STR(pret2.stax,10,2) AS 'TAX', STR(pret2.to_amt,10,2) AS 'Total', STR(pret1.chr_amt,10,2) AS 'Charge Amount', STR(pret1.dis_amt,10,2) AS 'Discount Amount', STR(pret1.adj_amt,10,2) AS 'Adjust Amount', STR(pret2.to_amt,10,2) AS 'TOTAL AMOUNT' FROM  pret1 LEFT OUTER JOIN  pret2 LEFT OUTER JOIN party ON pret2.prt_code = party.prt_code LEFT OUTER JOIN  item ON pret2.it_cd = item.it_cd ON pret1.pret_no = pret2.pret_no  WHERE (pret1.pret_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pret1.pret_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY " & orderby & "")
        Else
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY " & orderby & ") AS 'Sl.',  STR(pret1.pret_no,10,0) AS 'Purchase Return No.', CONVERT(varchar,pret1.pret_dt,103) AS 'Purchase Return Date', pret1.ref_no AS 'Reference No.', CONVERT(varchar,pret1.ref_dt,103) AS 'Reference Date', pret2.tr_tp AS 'Purchase Type', party.pname AS 'Party Name', item.it_name AS 'Product', STR(pret2.pret_qty,10,2) AS 'Quantity', STR(pret2.free_qty,10,2) AS 'Free', item.unt1 AS 'Unit', STR(pret2.mrp,10,2) AS 'M.R.P.', STR(pret2.pur_rt,10,2) AS 'Purchase Rate', STR(pret2.it_amt,10,2) AS 'Product Amount', STR(pret2.disc_amt1,10,2) AS 'Co. Dis. Amt.', STR(pret2.disc_amt2,10,2) AS 'Tr. Dis. Amt.',  STR(pret2.stax,10,2) AS 'TAX', STR(pret2.to_amt,10,2) AS 'Total', STR(pret1.chr_amt,10,2) AS 'Charge Amount', STR(pret1.dis_amt,10,2) AS 'Discount Amount', STR(pret1.adj_amt,10,2) AS 'Adjust Amount', STR(pret2.to_amt,10,2) AS 'TOTAL AMOUNT' FROM  pret1 LEFT OUTER JOIN  pret2 LEFT OUTER JOIN party ON pret2.prt_code = party.prt_code LEFT OUTER JOIN  item ON pret2.it_cd = item.it_cd ON pret1.pret_no = pret2.pret_no  WHERE (pret1.pret_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pret1.pret_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY " & orderby & "")
        End If
        If ds.Tables(0).Rows.Count <> 0 Then
            ...
            Next
            GroupBox3.Text = "Purchase Return Register From " & fromdt.Value & " To " & todt.Value
            hddr = GroupBox3.Text
        Else
            lblmsg.Visible = True
        End If
