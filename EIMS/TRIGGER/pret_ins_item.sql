CREATE TRIGGER pret_ins_item ON pret2
AFTER INSERT AS DECLARE @itcd BIGINT, @recstk MONEY, @prdtp INT
SELECT @itcd = it_cd, @recstk = pret_qty, @prdtp = prd_tp FROM inserted
BEGIN
 IF @prdtp = 1
  BEGIN
