...
Public Class frmjrn
    ...
    Private Sub frmjrn_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuJrn_Vouc", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            comd = "V"
            btnsave.Text = "View"
            Me.Text = "Journal Voucher View Screen . . ."
            slno = dv.SelectedRows(0).Cells(0).Value
            dv.Visible = False
            Me.dv_edit(slno)
        End If
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        ...
        If (prtnm Is Nothing OrElse prtnm = "") Then
            'User hit cancel
        Else
            Dim dssear As DataSet = get_dataset("SELECT jrn2.jr_no AS 'Jr. No', CONVERT(VARCHAR,jrn2.jr_dt,103) AS 'Date', party.pname AS 'Party Head Of A/c Name',STR((CASE WHEN jr_type='C' THEN jrn2.amt END),10,2) AS 'Cr. Amt',STR((CASE WHEN jr_type='D' THEN jrn2.amt END),10,2) AS 'Dr. Amt', jrn1.desc1 AS 'Remarks', jrn1.rec_lock FROM jrn2 RIGHT OUTER JOIN jrn1 ON jrn2.jr_no = jrn1.jr_no LEFT OUTER JOIN party ON jrn2.prt_code = party.prt_code WHERE (party.pname LIKE'" & prtnm & "%') ORDER BY jrn1.jr_no")
            dv.Columns.Clear()
