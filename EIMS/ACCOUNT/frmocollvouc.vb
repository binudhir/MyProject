...
Imports System.IO
Public Class frmocollvouc
    Dim comd As String = "E"
    Dim used As String = "N"
    Dim con_img As New SqlConnection(connectionstrng)
    Dim cmd_img As SqlCommand

    Private Sub frmocollvouc_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        cnfr = MessageBox.Show("Are You Sure To Close The Screen", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
        If cnfr = vbYes Then
            e.Cancel = False
            MDI.mnuFeeColl_NSC.Enabled = True
        Else
            e.Cancel = True
        End If
    End Sub

    Private Sub frmocollvouc_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmocollvouc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        MDI.mnuFeeColl_NSC.Enabled = False
        usrprmsn("mnuFeeColl_NSC", cmnuadd, cmnuedit, cmenudel, cmenuview)
        colldt.Value = sys_date
        Me.clr()
        Me.dv_disp()
    End Sub

    Private Sub frmocollvouc_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub clr()
        ...
        dvrec.Rows.Clear()
        If comd = "E" Then
            Me.Text = "Non Scheduled Collection Entry Screen . . ."
            Dim ds As DataSet = get_dataset("SELECT max(coll_no) as 'Max' FROM ocollrec1")
            txtcollno.Text = 1
            ...
            btnsave.Text = "Save"
        ElseIf comd = "M" Then
            Me.Text = "Non Scheduled Collection Modification Screen . . ."
            txtcollno.Text = ""
            btnsave.Text = "Modify"
        Else
            Me.Text = "Non Scheduled Collection Deletion Screen . . ."
            txtcollno.Text = ""
            btnsave.Text = "Delete"
        End If
        txtcollno.Focus()
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        ...
        If (prtnm Is Nothing OrElse prtnm = "") Then
            'User hit cancel
        Else
            Dim dssear As DataSet = get_dataset("SELECT ocollrec1.coll_no AS 'Coll. No.', CONVERT(VARCHAR,ocollrec1.coll_dt,103) AS 'Date', stud_hstry.reg_no AS 'Regd. No.',stud.stud_nm AS 'Stud. Name', sesion1.sesn_nm AS 'Session',trad.trad_nm AS 'Branch', (CASE WHEN ocollrec1.pay_mod='1' THEN 'Cash' WHEN ocollrec1.pay_mod='2' THEN 'DD/Cheque' END) AS 'Pay Mode' ,STR(ocollrec1.tot_amt,10,2) AS 'Amount' FROM stud RIGHT OUTER JOIN stud_hstry ON stud.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd RIGHT OUTER JOIN collrec1 ON stud_hstry.stud_sl = ocollrec1.stud_sl WHERE (stud_hstry.active = 'Y') AND (stud_hstry.reg_no LIKE'" & prtnm & "%') ORDER BY stud_hstry.reg_no")
            dv.Columns.Clear()
    ...
    Private Sub pay_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO ocollrec1 (coll_no,coll_dt,rec_tp,pay_mod,stud_sl,prt_code,tot_amt,bank_nm,chq_no,chq_dt,nb," & _
                          "usr_ent,ent_date,usr_mod,mod_date,rec_lock) VALUES (" & Val(txtcollno.Text) & ",'" & _
        ...
        ElseIf comd = "M" Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT coll_no FROM ocollrec1 WHERE coll_no=" & Val(txtcollno.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE ocollrec1 SET coll_dt='" & Format(colldt.Value, "dd/MMM/yyyy") & "',pay_mod='" & _
                              ...
                              Val(usr_sl) & ",mod_date='" & Format(Now, "dd/MMM/yyyy HH:mm:ss") & "' WHERE coll_no =" & Val(txtcollno.Text) & "")
                    Me.dv_save()
    ...
    Private Sub sms_send()
        ...
        If ds.Tables(0).Rows(0).Item("sms_req") = "Y" Then
            If coll_sms = 1 Then
                ...
                msg = "Dear " & Trim(fstnm) & ", Thanx For Payment Of Rs: " & Val(txttotamt.Text) & " Against Coll No.(O)" & Val(txtcollno.Text) & ". From : " & Trim(cosnm) & "."
                ...
                If stud_sms = 1 Then
                    ...
                    If Len(studmob) = 10 Then
                        saving_sms(mxno, "Other Collection SMS", studmob, msg)
                        mxno += 1
                    End If
                End If
                ...
                If fath_sms = 1 Then
                    ...
                    If Len(fatmob) = 10 Then
                        saving_sms(mxno, "Other Collection SMS", fatmob, msg)
                        mxno += 1
                    End If
                End If
                ...
                If moth_sms = 1 Then
                    ...
                    If Len(motmob) = 10 Then
                        saving_sms(mxno, "Other Collection SMS", motmob, msg)
                        mxno += 1
                    End If
                End If
                ...
                If grdn_sms = 1 Then
                    ...
                    If Len(grdnmob) = 10 Then
                        saving_sms(mxno, "Other Collection SMS", grdnmob, msg)
    ...
    Private Sub dv_save()
        If dvcoll.RowCount <> 0 Then
            ...
            For i As Integer = 0 To dvcoll.RowCount - 1
                ...
                Dim ds1 As DataSet = get_dataset("SELECT max(coll_sl) as 'Max' FROM ocollrec2")
                If Val(col_amt) <> 0 Then
                    ...
                    If ds1.Tables(0).Rows.Count <> 0 Then
                        ...
                        End If
                    End If
                    SQLInsert("INSERT INTO ocollrec2(coll_sl,coll_no,coll_dt,stud_sl,rec_tp,pay_mod,sem_cd,coll_cd,coll_amt) VALUES (" & Val(mxno) & "," & _
...
#Region "DISPLAY"
    ...
    Private Sub coll_view()
        Dim ds As DataSet = get_dataset("SELECT coll_nm,coll_cd FROM coll WHERE (schd = 'N') ORDER BY coll_nm")
        rw = 0
        dvcoll.Rows.Clear()
        If ds.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
    ...
    Private Sub colrec_view()
        If comd = "E" Then
            Dim ds As DataSet = get_dataset("SELECT coll.coll_nm, semester.sem_nm, ocollrec2.coll_amt FROM ocollrec2 LEFT OUTER JOIN semester ON ocollrec2.sem_cd = semester.sem_cd LEFT OUTER JOIN coll ON ocollrec2.coll_cd = coll.coll_cd WHERE(ocollrec2.stud_sl = " & Val(txtstudsl.Text) & ") ORDER BY ocollrec2.coll_sl")
            rw = 0
            amt = 0
            dvrec.Rows.Clear()
            If ds.Tables(0).Rows.Count <> 0 Then
            ...
            txtbalance.Text = Format(Val(txtfinlamt.Text) - amt, "#######0.00")
        Else
            Me.coll_view()
            Dim ds As DataSet = get_dataset("SELECT coll.coll_nm, semester.sem_nm, ocollrec2.coll_amt FROM ocollrec2 LEFT OUTER JOIN semester ON ocollrec2.sem_cd = semester.sem_cd LEFT OUTER JOIN coll ON ocollrec2.coll_cd = coll.coll_cd WHERE(ocollrec2.coll_no = " & Val(txtcollno.Text) & ") ORDER BY ocollrec2.coll_sl")
            rw = 0
            amt = 0
            dvrec.Rows.Clear()
            If ds.Tables(0).Rows.Count <> 0 Then
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT ocollrec1.coll_no AS 'Coll. No.', CONVERT(VARCHAR,ocollrec1.coll_dt,103) AS 'Date', stud_hstry.reg_no AS 'Regd. No.',stud.stud_nm AS 'Stud. Name', sesion1.sesn_nm AS 'Session',trad.trad_nm AS 'Branch', (CASE WHEN ocollrec1.pay_mod='1' THEN 'Cash' WHEN ocollrec1.pay_mod='2' THEN 'DD/Cheque' END) AS 'Pay Mode' ,STR(ocollrec1.tot_amt,10,2) AS 'Amount' FROM stud RIGHT OUTER JOIN stud_hstry ON stud.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd RIGHT OUTER JOIN ocollrec1 ON stud_hstry.stud_sl = ocollrec1.stud_sl WHERE (stud_hstry.active = 'Y') ORDER BY stud_hstry.reg_no")
        'Dim dsedit As DataSet = get_dataset("SELECT ocollrec1.coll_no, ocollrec1.tot_amt, trad.trad_nm, sesion1.sesn_nm, stud.stud_nm, stud_hstry.reg_no, ocollrec1.coll_dt, ocollrec1.pay_mod FROM stud RIGHT OUTER JOIN stud_hstry ON stud.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd RIGHT OUTER JOIN ocollrec1 ON stud_hstry.stud_sl = ocollrec1.stud_sl WHERE (stud_hstry.active = 'Y') ORDER BY stud_hstry.reg_no")
        dv.Columns.Clear()
        ...
        '        dv.Rows.Add()
        '        dv.Rows(rw).Cells(0).Value = dsedit.Tables(0).Rows(i).Item("coll_no")
        '        dv.Rows(rw).Cells(1).Value = Format(dsedit.Tables(0).Rows(i).Item("coll_dt"), "dd/MM/yyyy")
        '        dv.Rows(rw).Cells(2).Value = dsedit.Tables(0).Rows(i).Item("reg_no")
        '        dv.Rows(rw).Cells(3).Value = dsedit.Tables(0).Rows(i).Item("stud_nm")
        '        dv.Rows(rw).Cells(4).Value = dsedit.Tables(0).Rows(i).Item("sesn_nm")
        '        dv.Rows(rw).Cells(5).Value = dsedit.Tables(0).Rows(i).Item("trad_nm")
        '        If dsedit.Tables(0).Rows(i).Item("pay_mod") = "1" Then
        '            dv.Rows(rw).Cells(6).Value = "Cash"
        '        Else
        '            dv.Rows(rw).Cells(6).Value = "DD/Cheque"
        '        End If
        '        dv.Rows(rw).Cells(7).Value = Format(dsedit.Tables(0).Rows(i).Item("tot_amt"), "######0.00")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT stud_hstry.reg_no, stud_hstry.sesn_cd, stud_hstry.sem_cd, stud.stud_nm, trad.trad_nm, sesion1.sesn_nm, sesion1.sesn_dt1, sesion1.sesn_dt2, semester.sem_nm, ocollrec1.*, party.pname,DataLength(stud.stud_image) AS stud_img FROM party RIGHT OUTER JOIN ocollrec1 ON party.prt_code = ocollrec1.prt_code LEFT OUTER JOIN stud_hstry LEFT OUTER JOIN semester ON stud_hstry.sem_cd = semester.sem_cd ON ocollrec1.stud_sl = stud_hstry.stud_sl LEFT OUTER JOIN sesion1 ON stud_hstry.sesn_cd = sesion1.sesn_cd LEFT OUTER JOIN trad ON stud_hstry.trad_cd = trad.trad_cd LEFT OUTER JOIN stud ON stud_hstry.stud_sl = stud.stud_sl WHERE (stud_hstry.active = 'Y') AND (ocollrec1.coll_no = " & slno & ")")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtcollno.Text = slno
...
#End Region
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            comd = "V"
            btnsave.Text = "View"
            Me.Text = "Non Scheduled Collection View Screen . . ."
            slno = dv.SelectedRows(0).Cells(0).Value
            dv.Visible = False
            Me.dv_edit(slno)
        End If
    ...
    Private Sub dvcoll_CellValidated(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dvcoll.CellValidated
        ...
        Next
        txttotamt.Text = Format(amt, "########0.00")
        txtbalance.Text = Format(Val(txtfinlamt.Text) - (Val(txtpaidamt.Text) + amt), "########0.00")
    End Sub
    ...
    Private Sub txtcollno_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtcollno.Validated
        If comd = "M" Then
            Dim ds1 As DataSet = get_dataset("SELECT coll_no FROM ocollrec1 WHERE (coll_no = '" & Trim(txtcollno.Text) & "')")
            If ds1.Tables(0).Rows.Count <> 0 Then
    ...
    Private Sub del1(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT coll_no FROM ocollrec1 WHERE coll_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            SQLInsert("DELETE FROM ocollrec1 WHERE coll_no=" & Val(code) & "")
        End If
    End Sub

    'Delete data from Table2/Secondary Table
    Private Sub del2(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT coll_sl FROM ocollrec2 WHERE coll_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To dsdel.Tables(0).Rows.Count - 1
                posl = dsdel.Tables(0).Rows(i).Item(0)
                SQLInsert("DELETE FROM ocollrec2 WHERE coll_sl=" & Val(posl) & "")
            Next
        End If
    End Sub
