...
Public Class frmrecvouc
    ...
    Private Sub frmrecvouc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        usrprmsn("mnuRec_Vouc", cmnuadd, cmnuedit, cmenudel, cmenuview)
        Me.clr()
        Me.dv_disp()
    End Sub
    ...
    Private Sub clr()
        ...
        Dim ds As DataSet = get_dataset("SELECT max(v_no) as 'Max' FROM voucr_coll")
        txtvno.Text = 1
    ...
    Private Sub btnsave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.Click
        ...
        If Trim(cmbcollby.Text) = "" Or Val(txtcollcd.Text) = 0 Then
            MessageBox.Show("Sorry The Recieved By Should not be Blank.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            cmbcollby.Focus()
            Exit Sub
        End If
        If Val(txtamt.Text) = 0 Then
    ...
    Private Sub pay_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO voucr_coll (v_no,v_dt,prt_code,amt,staf_sl,desc1,desc2,desc3,pay_mode,by_code," & _
        ...
        ElseIf comd = "M" Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT v_no FROM voucr_coll WHERE v_no=" & Val(txtvno.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE voucr_coll SET v_dt='" & Format(vdt.Value, "dd/MMM/yyyy") & "',prt_code=" & Val(txtprtcd.Text) & ",amt=" & _
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT voucr_coll.v_no AS 'Vc.No', CONVERT(VARCHAR,voucr_coll.v_dt,103) AS 'Date', party.pname AS 'Party Head Of A/c Name',(CASE WHEN voucr_coll.pay_mode='1' THEN 'Cash' WHEN voucr_coll.pay_mode='2' THEN 'DD/Cheque' END) AS 'Mode',  STR(voucr_coll.amt,10,2) AS 'Amount', voucr_coll.desc1 AS 'Remarks', voucr_coll.rec_lock FROM voucr_coll LEFT OUTER JOIN party ON voucr_coll.prt_code = party.prt_code ORDER BY voucr_coll.v_no")
        dv.Columns.Clear()
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT voucr_coll.*, party.pname,party.add1,party.cl_amt,staf.short_nm FROM voucr_coll LEFT OUTER JOIN staf ON voucr_coll.staf_sl = staf.staf_sl LEFT OUTER JOIN party ON voucr_coll.prt_code = party.prt_code WHERE voucr_coll.v_no=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtvno.Text = slno
    ...
    Private Sub cmenuview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenuview.Click
        If dv.RowCount <> 0 Then
            comd = "V"
            btnsave.Text = "View"
            Me.Text = "Receipt Voucher View Screen . . ."
            slno = dv.SelectedRows(0).Cells(0).Value
            dv.Visible = False
            Me.dv_edit(slno)
        End If
    ...
    Private Sub cmenudel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenudel.Click
        ...
        If dv.RowCount <> 0 Then
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT v_no,amt,prt_code,by_code FROM voucr_coll WHERE v_no=" & Val(slno) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    If fnd = 0 Then
                        ...
                        End If
                        SQLInsert("DELETE FROM voucr_coll WHERE v_no =" & Val(slno) & "")
    ...
    Private Sub cmbrecfrm_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbrecfrm.Validated
        Dim ds As DataSet = get_dataset("SELECT prt_code,add1 FROM party WHERE pname='" & Trim(cmbrecfrm.Text) & "'")
        If ds.Tables(0).Rows.Count <> 0 Then
    ...
    Private Sub cmenusearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenusearch.Click
        ...
        If (prtnm Is Nothing OrElse prtnm = "") Then
            'User hit cancel
        Else
            Dim dssear As DataSet = get_dataset("SELECT voucr_coll.v_no AS 'Vc.No', CONVERT(VARCHAR,voucr_coll.v_dt,103) AS 'Date', party.pname AS 'Party Head Of A/c Name',(CASE WHEN voucr_coll.pay_mode='1' THEN 'Cash' WHEN voucr_coll.pay_mode='2' THEN 'DD/Cheque' END) AS 'Mode',  STR(voucr_coll.amt,10,2) AS 'Amount', voucr_coll.desc1 AS 'Remarks', voucr_coll.rec_lock FROM voucr_coll LEFT OUTER JOIN party ON voucr_coll.prt_code = party.prt_code WHERE (party.pname LIKE'" & prtnm & "%') ORDER BY voucr_coll.v_no")
            dv.Columns.Clear()
