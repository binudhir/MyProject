...
Module ordprint
    ...
    Private Sub datastore()
        ...
        Dim ds1 As DataSet = get_dataset("SELECT     porder1.po_no, porder1.po_dt, porder1.deliver_dt, porder1.nb1, party.pname, party.add1, party.cont1, item.it_name, porder2.po_qty, porder2.po_mrp, porder2.po_unit, porder1.gr_tot, porder2.po_rate, porder2.po_amt FROM item RIGHT OUTER JOIN porder2 ON item.it_cd = porder2.it_cd RIGHT OUTER JOIN porder1 ON porder2.po_no = porder1.po_no LEFT OUTER JOIN party ON porder1.prt_code = party.prt_code WHERE porder1.po_no=" & Val(ord_no) & "")
        If ds1.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To ds1.Tables(0).Rows.Count - 1
                ...
                nb = ds1.Tables(0).Rows(i).Item("nb1")
                ...
                tot = ds1.Tables(0).Rows(i).Item("gr_tot")
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO print_order(prnt_no,supl_nm,add1,cont1,po_no,ordr_dt,delv_dt,prod_nm,po_qty,nb,unt,mrp,po_rate,po_amt,gr_tot,inword," & _
                          "usr_sl,comp_nm,comp_add1,comp_add2,comp_cont1,comp_email,comp_web) VALUES (" & mxno & ",'" & _
                          suplnm & "','" & add1 & "','" & cont & "'," & pono & ",'" & orddt & "','" & deldt & "','" & _
                         itnm & "'," & qty & ",'" & nb & "','" & unt & "','" & mrp & "','" & purt & "','" & amt & "','" & tot & "','" & inword & "'," & usr_sl & _
                         ",'" & conm & "','" & coadd & "','" & coadd & "','" & coph1 & "','" & coemail & "','" & coweb & "')")
            Next
        End If
    End Sub
