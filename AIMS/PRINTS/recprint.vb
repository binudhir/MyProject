...
Module recprint
    ...
    Public Sub rvouc_print(ByVal slno As Integer)
        vno = slno
        Call datastore()
        Select Case printrvouc
            Case Is = 1
                Call recvouc1()  ' Windows Print Voucher Default Half Page
            Case Else
                Call recvouc1()
        End Select
        For i As Integer = 0 To copyrvouc
            Call print_repo()
        Next
    End Sub
    ...
    Private Sub datastore()
        ...
        Dim ds1 As DataSet = get_dataset("SELECT vouc.*, staf.staf_nm, party.pname,party.add1,party.cl_amt, (SELECT pname FROM party WHERE party.prt_code=vouc.by_code) AS bank_nm,(SELECT add1 FROM party WHERE party.prt_code=vouc.by_code) AS bank_add FROM vouc LEFT OUTER JOIN party ON vouc.prt_code = party.prt_code LEFT OUTER JOIN staf ON vouc.staf_sl = staf.staf_sl WHERE vouc.v_no=" & Val(vno) & "")
    ...
    Private Sub recvouc1()
        reportnm = "recvouc.rpt"
    End Sub
End Module
