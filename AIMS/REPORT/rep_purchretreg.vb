...
Public Class rep_purchretreg
    Dim comd As String

    Private Sub rep_purchretreg_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_purchretreg_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_purchretreg_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#End Region
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        If cmbtype.SelectedIndex = 1 Then
            conditn = " AND (pret2.prd_tp = '1')"
        ElseIf cmbtype.SelectedIndex = 2 Then
            conditn = " AND (pret2.prd_tp = '2')"
        End If
        orderby = "pret1.pret_dt"
        ...
        If (chkdetail.Checked = True) Then
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY it_name) AS 'Sl.', pret1.pret_no AS 'Return No.', CONVERT(varchar, pret1.pret_dt,103) AS 'Return Dt.', party.pname AS 'Party Name', pret1.ref_no AS 'Ref No.', CONVERT(varchar,pret1.ref_dt,103) AS 'Ref Dt.', item.it_name AS 'Product Name', manf.manf_nm AS 'Company Name', batno.bat_no AS 'Batch No.',(CASE WHEN pret2.prd_tp='1' THEN 'Snd' WHEN pret2.prd_tp='2' THEN 'Dmg' END) AS 'Type', STR(pret2.pret_qty,10,2) AS 'Quantity', STR(pret2.free_qty,10,2) AS 'Free', pret2.unt AS 'Unit', STR(pret2.mrp,10,2) AS 'M.R.P', STR(pret2.pur_rt,10,2) AS 'Purch Rate', STR(pret2.tto_amt,10,2) AS 'Amount', taxper.taxnm AS 'Tax Name', STR(pret2.stax,10,2) AS 'Tax Amt', STR(pret2.it_tot,10,2) AS 'Total Amt' FROM taxper RIGHT OUTER JOIN pret2 ON taxper.taxcd = pret2.tax_cd LEFT OUTER JOIN batno ON pret2.bat_cd = batno.bat_cd LEFT OUTER JOIN item LEFT OUTER JOIN manf ON item.manf_cd = manf.manf_cd ON pret2.it_cd = item.it_cd RIGHT OUTER JOIN pret1 ON pret2.pret_no = pret1.pret_no LEFT OUTER JOIN party ON pret1.prt_code = party.prt_code WHERE (pret1.pret_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pret1.pret_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY party.pname,item.it_name")
        Else
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY it_name) AS 'Sl.', pret1.pret_no AS 'Return No.', CONVERT(varchar, pret1.pret_dt,103) AS 'Return Dt', party.pname AS 'Party Name', pret1.ref_no AS 'Ref No.', CONVERT(varchar,pret1.ref_dt,103) AS 'Ref Dt', item.it_name AS 'Product Name', manf.manf_nm AS 'Company Name', batno.bat_no AS 'Batch No',(CASE WHEN pret2.prd_tp='1' THEN 'Snd' WHEN pret2.prd_tp='2' THEN 'Dmg' END) AS 'Type', STR(pret2.pret_qty,10,2) AS 'Quantity', STR(pret2.free_qty,10,2) AS 'Free', pret2.unt AS 'Unit', STR(pret2.mrp,10,2) AS 'M.R.P', STR(pret2.pur_rt,10,2) AS 'Purch Rate', STR(pret2.tto_amt,10,2) AS 'Amount', taxper.taxnm AS 'Tax Name', STR(pret2.stax,10,2) AS 'Tax Amt', STR(pret2.it_tot,10,2) AS 'Total Amt' FROM taxper RIGHT OUTER JOIN pret2 ON taxper.taxcd = pret2.tax_cd LEFT OUTER JOIN batno ON pret2.bat_cd = batno.bat_cd LEFT OUTER JOIN item LEFT OUTER JOIN manf ON item.manf_cd = manf.manf_cd ON pret2.it_cd = item.it_cd RIGHT OUTER JOIN pret1 ON pret2.pret_no = pret1.pret_no LEFT OUTER JOIN party ON pret1.prt_code = party.prt_code WHERE (pret1.pret_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pret1.pret_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY party.pname,item.it_name")
        End If
        If ds.Tables(0).Rows.Count <> 0 Then
            ...
            '    If ds.Tables(0).Rows(i).Item("Purchase type") = "1" The
            ...
            'Next
            GroupBox3.Text = " Purchase Return Register Form " & fromdt.Value & " To " & todt.Value
            hddr = GroupBox3.Text
        Else
            lblmsg.Visible = True
        End If
