...
Public Class rep_purchchalreg
    Dim comd As String

    Private Sub rep_purchchalreg_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub rep_purchchalreg_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub rep_purchchalreg_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#End Region
    ...
    Private Sub btnview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnview.Click
        ...
        If cmbtype.SelectedIndex = 1 Then
            conditn = " AND (pchal2.pch_qty-pchal2.bpch_qty = 0)"
        ElseIf cmbtype.SelectedIndex = 2 Then
            conditn = " AND (pchal2.pch_qty-pchal2.bpch_qty <> 0)"
        End If
        orderby = "pchal1.pch_dt"
        ...
        If (chkdetail.Checked = True) Then
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY it_name) AS 'Sl.', pchal1.pch_no AS 'Challan No.', CONVERT(varchar, pchal1.pch_dt,103) AS 'Challan Dt', party.pname AS 'Party Name', pchal1.ref_no AS 'Refrence No.', CONVERT(varchar,pchal1.ref_dt,103) AS 'Refrence Dt', item.it_name AS 'Product Name ', manf.manf_nm AS 'Company Name', batno.bat_no AS 'Batch No', (CASE WHEN pchal2.prd_tp='1' THEN 'S' WHEN pchal2.prd_tp='2' THEN 'D' END) AS 'Product Type', pchal2.unt AS 'Unit', STR(pchal2.mrp,10,2) AS 'MRP', STR(pchal2.pch_qty,10,2) AS 'Quantity',STR(pchal2.bpch_qty,10,2) AS 'Billed Qty', STR(pchal2.free_qty,10,2) AS 'Free',STR(pchal2.bfree_qty,10,2) AS 'Billed Free', STR(pchal2.pur_rt,10,2) AS 'Purchase Rate', STR(pchal2.it_amt,10,2) AS 'Total' FROM batno RIGHT OUTER JOIN pchal2 ON batno.bat_cd = pchal2.bat_cd LEFT OUTER JOIN item LEFT OUTER JOIN manf ON item.manf_cd = manf.manf_cd ON pchal2.it_cd = item.it_cd RIGHT OUTER JOIN pchal1 ON pchal2.pch_no = pchal1.pch_no LEFT OUTER JOIN party ON pchal1.prt_code = party.prt_code WHERE (pchal1.pch_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pchal1.pch_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY party.pname,item.it_name")
        Else
            ds = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY it_name) AS 'Sl.', pchal1.pch_no AS 'Challan No.', CONVERT(varchar, pchal1.pch_dt,103) AS 'Challan Dt', party.pname AS 'Party Name', pchal1.ref_no AS 'Refrence No.', CONVERT(varchar,pchal1.ref_dt,103) AS 'Refrence Dt', item.it_name AS 'Product Name ', manf.manf_nm AS 'Company Name', batno.bat_no AS 'Batch No', (CASE WHEN pchal2.prd_tp='1' THEN 'S' WHEN pchal2.prd_tp='2' THEN 'D' END) AS 'Product Type', pchal2.unt AS 'Unit', STR(pchal2.mrp,10,2) AS 'MRP', STR(pchal2.pch_qty,10,2) AS 'Quantity',STR(pchal2.bpch_qty,10,2) AS 'Billed Qty', STR(pchal2.free_qty,10,2) AS 'Free',STR(pchal2.bfree_qty,10,2) AS 'Billed Free', STR(pchal2.pur_rt,10,2) AS 'Purchase Rate', STR(pchal2.it_amt,10,2) AS 'Total' FROM batno RIGHT OUTER JOIN pchal2 ON batno.bat_cd = pchal2.bat_cd LEFT OUTER JOIN item LEFT OUTER JOIN manf ON item.manf_cd = manf.manf_cd ON pchal2.it_cd = item.it_cd RIGHT OUTER JOIN pchal1 ON pchal2.pch_no = pchal1.pch_no LEFT OUTER JOIN party ON pchal1.prt_code = party.prt_code WHERE (pchal1.pch_dt>='" & Format(fromdt.Value, "dd/MMM/yyyy") & "') AND (pchal1.pch_dt<='" & Format(todt.Value, "dd/MMM/yyyy") & "')  " & conditn & " ORDER BY party.pname,item.it_name")
        End If
        If ds.Tables(0).Rows.Count <> 0 Then
            ...
            'Next
            GroupBox3.Text = " Challan Register Form " & fromdt.Value & " To " & todt.Value
            hddr = GroupBox3.Text
        Else
            lblmsg.Visible = True
        End If
