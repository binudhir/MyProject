...
Public Class frmpayvouc
    ...
    Private Sub clr()
        ...
        Dim ds As DataSet = get_dataset("SELECT max(v_no) as 'Max' FROM voucp")
        txtvno.Text = 1
    ...
    Private Sub pay_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO voucp (v_no,v_dt,prt_code,amt,staf_sl,desc1,desc2,desc3,pay_mode,by_code," & _
                ...
                cnfr = MessageBox.Show("Are You Sure The Printer Is Ready To Print The Voucher.", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
                If cnfr = vbYes Then
                    Call pvouc_print(Val(txtvno.Text))
                End If
                Me.clr()
                Close1()
            End If
        Else
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT v_no FROM voucp WHERE v_no=" & Val(txtvno.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE voucp SET v_dt='" & Format(vdt.Value, "dd/MMM/yyyy") & "',prt_code=" & Val(txtprtcd.Text) & ",amt=" & _
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT voucp.v_no AS 'Vc.No', CONVERT(VARCHAR,voucp.v_dt,103) AS 'Date', party.pname AS 'Party Head Of A/c Name',(CASE WHEN voucp.pay_mode='1' THEN 'Cash' WHEN voucp.pay_mode='2' THEN 'DD/Cheque' END) AS 'Mode',  STR(voucp.amt,10,2) AS 'Amount', voucp.desc1 AS 'Remarks', voucp.rec_lock FROM voucp LEFT OUTER JOIN party ON voucp.prt_code = party.prt_code ORDER BY voucp.v_no DESC")
        dv.Columns.Clear()
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT voucp.*, party.pname,party.add1,party.cl_amt,staf.short_nm FROM voucp LEFT OUTER JOIN staf ON voucp.staf_sl = staf.staf_sl LEFT OUTER JOIN party ON voucp.prt_code = party.prt_code WHERE voucp.v_no=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtvno.Text = slno
    ...
    Private Sub cmenudel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenudel.Click
        ...
        If dv.RowCount <> 0 Then
            If usr_tp = "A" Or usr_tp = "D" Then
                ...
                Start1()
                If cnfr = vbYes Then
                    Dim ds As DataSet = get_dataset("SELECT v_no,amt,prt_code,by_code FROM voucp WHERE v_no=" & Val(slno) & "")
                    If ds.Tables(0).Rows.Count <> 0 Then
                        ...
                        If fnd = 0 Then
                            ...
                            End If
                            SQLInsert("DELETE FROM voucp WHERE v_no =" & Val(slno) & "")
