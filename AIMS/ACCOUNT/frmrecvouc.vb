...
Public Class frmrecvouc
    Dim comd As String

    Private Sub frmrecvouc_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        cnfr = MessageBox.Show("Are You Sure To Close The Screen", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
        If cnfr = vbYes Then
            e.Cancel = False
            MDI.mnuRec_Vouc.Enabled = True
        Else
            e.Cancel = True
        End If
    End Sub

    Private Sub frmrecvouc_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmrecvouc_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        MDI.mnuRec_Vouc.Enabled = False
        Me.clr()
        Me.dv_disp()
        vdt.Value = sys_date
    End Sub

    Private Sub frmrecvouc_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub clr()
        Me.Text = "Receipt Voucher Entry Screen . . ."
        lbladdress.Text = ""
        lbladdress1.Text = ""
        ...
        Dim ds As DataSet = get_dataset("SELECT max(v_no) as 'Max' FROM vouc")
        txtvno.Text = 1
    ...
    Private Sub btnexit_MouseEnter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdview.MouseEnter, btnsave.MouseEnter, btns1.MouseEnter, btnrefresh.MouseEnter, btnexit.MouseEnter, btnprint.MouseEnter
        sender.forecolor = Color.Violet
    End Sub

    Private Sub btnexit_MouseLeave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdview.MouseLeave, btnsave.MouseLeave, btns1.MouseLeave, btnrefresh.MouseLeave, btnexit.MouseLeave, btnprint.MouseLeave
    ...
    Private Sub pay_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO vouc (v_no,v_dt,prt_code,amt,staf_sl,desc1,desc2,desc3,pay_mode,by_code," & _
                ...
                SQLInsert("UPDATE party SET cr_amt=cr_amt + " & Val(txtamt.Text) & ",cl_amt=cl_amt + " & Val(txtamt.Text) & " WHERE prt_code=" & Val(txtprtcd.Text) & "")
                ...
                cnfr = MessageBox.Show("Are You Sure The Printer Is Ready To Print The Receipt Voucher.", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
                If cnfr = vbYes Then
                    Call rvouc_print(Val(txtvno.Text))
                End If
                Me.clr()
                Close1()
            End If
        Else
            ...
            Start1()
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT v_no FROM vouc WHERE v_no=" & Val(txtvno.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE vouc SET v_dt='" & Format(vdt.Value, "dd/MMM/yyyy") & "',prt_code=" & Val(txtprtcd.Text) & ",amt=" & _
                    ...
                    SQLInsert("UPDATE party SET cr_amt=cr_amt - " & Val(lblamt.Text) & ",cl_amt=cl_amt - " & Val(lblamt.Text) & " WHERE prt_code=" & Val(txtprtcd.Text) & "")
                    If Val(txtbycd.Text) <> 0 Then
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT vouc.v_no AS 'Vc.No', CONVERT(VARCHAR,vouc.v_dt,103) AS 'Date', party.pname AS 'Party Head Of A/c Name',(CASE WHEN vouc.pay_mode='1' THEN 'Cash' WHEN vouc.pay_mode='2' THEN 'DD/Cheque' END) AS 'Mode',  STR(vouc.amt,10,2) AS 'Amount', vouc.desc1 AS 'Remarks', vouc.rec_lock FROM vouc LEFT OUTER JOIN party ON vouc.prt_code = party.prt_code ORDER BY vouc.v_no DESC")
        dv.Columns.Clear()
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT vouc.*, party.pname,party.add1,party.cl_amt,staf.short_nm FROM vouc LEFT OUTER JOIN staf ON vouc.staf_sl = staf.staf_sl LEFT OUTER JOIN party ON vouc.prt_code = party.prt_code WHERE vouc.v_no=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtvno.Text = slno
    ...
    Private Sub cmnuedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuedit.Click
        If dv.RowCount <> 0 Then
            comd = "M"
            btnsave.Text = "Edit"
            Me.Text = "Receipt Voucher Modification Screen . . ."
    ...
    Private Sub cmenudel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmenudel.Click
        ...
        If dv.RowCount <> 0 Then
            If usr_tp = "A" Or usr_tp = "D" Then
                ...
                Start1()
                If cnfr = vbYes Then
                    Dim ds As DataSet = get_dataset("SELECT v_no,amt,prt_code,by_code FROM vouc WHERE v_no=" & Val(slno) & "")
                    If ds.Tables(0).Rows.Count <> 0 Then
                        ...
                        If fnd = 0 Then
                            ...
                            End If
                            SQLInsert("DELETE FROM vouc WHERE v_no =" & Val(slno) & "")
    ...
    Private Sub cmbpname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbpname.Validating
        vdating(cmbpname, "SELECT pname FROM party WHERE prt_type<>'A' AND rec_lock='N' AND pname='" & Trim(cmbpname.Text) & "'", "Please Select A Valid Party Head Of A/c Name.")
    End Sub
