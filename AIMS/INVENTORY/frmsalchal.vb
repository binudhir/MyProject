...
Public Class frmsalchal
    Dim comd As String = "E"
    Private allowCoolMove As Boolean = False
    Private myCoolPoint As New Point
#Region "Start Up"
    Private Sub frmsalchall_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        cnfr = MessageBox.Show("Are You Sure To Close The Screen", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1)
        If cnfr = vbYes Then
            e.Cancel = False
            MDI.mnuschal.Enabled = True
        Else
            e.Cancel = True
        End If
    End Sub

    Private Sub frmsalchal_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmsalchal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        MDI.mnuschal.Enabled = False
        Me.clr()
    ...
    Private Sub frmsalchal_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
    ...
    Private Sub clr()
        Me.Text = "Sales Challan Entry Screen . . ."
        txtslno.Text = "1"
        cmbprt.Text = ""
        txtref.Text = "X"
        txtrealmrp.Text = 0
        ...
        If comd = "E" Then
            ...
            btnsave.Text = "Save"
        ElseIf comd = "M" Then
            Me.Text = "Sales Challan Modification Screen . . ."
            txtscrl.Text = ""
            btnsave.Text = "Modify"
        Else
            Me.Text = "Sales Challan Deletion Screen . . ."
            txtscrl.Text = ""
            btnsave.Text = "Delete"
        End If
        Me.clr1()
...
#End Region
#Region "Enter/Leave"
    Private Sub txtscrl_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtslno.Enter, txtscrl.Enter, txtref.Enter, txtquantity.Enter, txtmrp.Enter, cmbunit.Enter, cmbprt.Enter, cmbproduct.Enter, cmbpr.Enter, txtnb.Enter, txtlrno.Enter, txtwaypfx.Enter, txtwbno.Enter, txtordno.Enter, txtvehno.Enter, cmbstore.Enter, cmbbatch.Enter, txtfree.Enter, cmbtr.Enter, txtrate.Enter
        sender.forecolor = Color.Blue
        sender.backcolor = Color.LavenderBlush
    End Sub

    Private Sub txtscrl_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtslno.Leave, txtscrl.Leave, txtref.Leave, txtquantity.Leave, txtmrp.Leave, cmbunit.Leave, cmbprt.Leave, cmbproduct.Leave, cmbpr.Leave, txtnb.Leave, txtlrno.Leave, txtwaypfx.Leave, txtwbno.Leave, txtordno.Leave, txtvehno.Leave, cmbstore.Leave, cmbbatch.Leave, txtfree.Leave, cmbtr.Leave, txtrate.Leave
...
#Region "Combo Box"
    ...
    Private Sub cmbprt_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbprt.GotFocus
        lbladdress.Text = ""
        cmbprt.Height = 100
        populate(cmbprt, "SELECT pname FROM party WHERE prt_type='B' AND rec_lock='N' ORDER BY pname")
    End Sub
    ...
    Private Sub cmbprt_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbprt.Validating
        vdating(cmbprt, "SELECT pname FROM party WHERE prt_type='B' AND rec_lock='N' AND pname='" & Trim(cmbprt.Text) & "'", "Please Select A Valid Party Head Of A/c Name.")
    End Sub
...
#Region "Button"
    ...
    Private Sub btnsetaply_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsetaply.Click
        Start1()
        SQLInsert("UPDATE setting_schal SET req_mrp=" & IIf(chksetmrp.Checked, 1, 0) & ",req_rate=" & _
    ...
    Private Sub btnnxt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnnxt.Click
        ...
        If cmbtr.SelectedIndex = 0 Then
            If Val(lblsnd.Text) < (Val(txtquantity.Text * (-1)) + Val(txtfree.Text) * (-1)) Then
                MessageBox.Show("Sorry Not Sufficient Stock to Sales.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                txtquantity.Focus()
                Exit Sub
            End If
        End If
        ...
        If chksetrate.Checked = True Then
            ...
            SQLInsert("UPDATE item SET pur_rate=" & sale_rt & " WHERE it_cd=" & Val(txtitcd.Text) & "")
            SQLInsert("UPDATE batno SET pur_rate=" & sale_rt & " WHERE it_cd=" & Val(txtitcd.Text) & " AND bat_cd=" & Val(txtbatcd.Text) & "")
        End If
        Close1()
        cmbproduct.Focus()
        Me.grandamt(dv1)
...
#Region "Data save"
    Private Sub chal_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO schal1(sch_no,sch_dt,prt_code,ref_no,ref_dt,way_bno,way_chr,trk_no,lr_no,lr_dt,sch_amt,billed," & _
                          "usedby,rec_lock,nb,ord_no,ord_dt,god_sl,usr_ent,ent_date,usr_mod,mod_date,pfx) VALUES (" & Val(txtscrl.Text) & ",'" & _
                          ...
                          Format(Now, "dd/MMM/yyyy HH:mm:ss") & "'," & Val(usr_sl) & ",'" & Format(Now, "dd/MMM/yyyy HH:mm:ss") & "','AA')")
                Me.dv_save()
        ...
        ElseIf comd = "M" Then
            ...
            If cnfr = vbYes Then
                Dim ds As DataSet = get_dataset("SELECT sch_no FROM schal1 WHERE sch_no=" & Val(txtscrl.Text) & "")
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE schal1 SET sch_dt='" & Format(dtchal.Value, "dd/MMM/yyyy") & "', prt_code=" & _
                              ...
                              Format(lrdt.Value, "dd/MMM/yyyy") & "', sch_amt=" & Val(lbltotamt.Text) & ", nb='" & _
                              Trim(txtnb.Text) & "', ord_no='" & Trim(txtordno.Text) & "', ord_dt='" & _
                              Format(orddt.Value, "dd/MMM/yyyy") & "', god_sl=" & Val(txtgodcd.Text) & ", usr_mod=" & _
                              Val(usr_sl) & ", mod_date='" & Format(Now, "dd/MMM/yyyy HH:mm:ss") & "' WHERE sch_no =" & Val(txtscrl.Text) & "")
                    Me.dv_save()
    ...
    Private Sub dv_save()
        If dv1.RowCount <> 0 Then
            ...
            For i As Integer = 0 To dv1.RowCount - 1
                ...
                itamt = prate * qty
                Dim ds1 As DataSet = get_dataset("SELECT max(sch_sl) as 'Max' FROM schal2")
                mxno = 1
                If ds1.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO schal2(sch_sl,sch_no,sch_dt,sch_tp,prd_tp,prt_code,it_cd,bat_cd,sale_rt,sch_qty,free_qty,bsch_qty,bfree_qty," & _
        ...
        If dvsl1.RowCount <> 0 Then
            For m As Integer = 0 To dvsl1.RowCount - 1
                ...
                If Val(itcd) = Val(it_cd) Then
                    ...
                    SQLInsert("INSERT INTO schal_slno (sl_slno,it_cd,sch_no,sch_sl,billed,pur_no,pur_sl) VALUES (" & Val(srlcd) & "," & itcd & "," & Val(txtscrl.Text) & "," & Val(pchsl) & ",'N',0,0)")
                End If
            Next
        End If
    End Sub
...
#Region "Data Retrieve"
    Private Sub dv_disp()
        Dim dsedit As DataSet = get_dataset("SELECT schal1.sch_no AS 'Chal.No', schal1.sch_dt AS 'Challan Dt', party.pname AS 'Paty Name', schal1.ref_no AS 'Ref No', schal1.ref_dt AS 'Ref Dt', schal1.billed AS 'Billed' FROM schal1 LEFT OUTER JOIN party ON schal1.prt_code = party.prt_code ")
        dv.Columns.Clear()
    ...
    Private Sub dv_view()
        Dim ds As DataSet = get_dataset("SELECT schal2.*, item.it_name,batno.bat_no FROM schal2 LEFT OUTER JOIN batno ON schal2.bat_cd = batno.bat_cd LEFT OUTER JOIN item ON schal2.it_cd = item.it_cd WHERE schal2.sch_no=" & Val(txtscrl.Text) & "")
        rw = 0
        dv1.Rows.Clear()
        If ds.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                End If
                If ds.Tables(0).Rows(i).Item("sch_tp") = "1" Then
                ...
                If chkbilled.Checked = False Then
                    dv1.Rows(rw).Cells(6).Value = Format(ds.Tables(0).Rows(i).Item("sch_qty"), "####0.00")
                    dv1.Rows(rw).Cells(7).Value = Format(ds.Tables(0).Rows(i).Item("free_qty"), "####0.00")
                Else
                    dv1.Rows(rw).Cells(6).Value = Format(ds.Tables(0).Rows(i).Item("bsch_qty"), "####0.00")
                ...
                rw += 1
                'If txtbatcnfr.Text = "S" Then
                pchsl = ds.Tables(0).Rows(i).Item("sch_sl")
                Me.srl_view(pchsl)
                'End If
            Next
            Me.grandamt(dv1)
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        Dim dsedit As DataSet = get_dataset("SELECT schal1.*, party.pname,party.cl_amt,party.add1,godown.godnm FROM schal1  LEFT OUTER JOIN godown ON schal1.god_sl = godown.godsl LEFT OUTER JOIN party ON schal1.prt_code = party.prt_code WHERE schal1.sch_no=" & Val(slno) & "")
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscrl.Text = dsedit.Tables(0).Rows(0).Item("sch_no")
            ...
            dtchal.Value = dsedit.Tables(0).Rows(0).Item("sch_dt")
    ...
    Private Sub srl_view(ByVal pchsl As Integer)
        Dim ds2 As DataSet = get_dataset("SELECT mast_slno.slno, schal_slno.* FROM schal_slno LEFT OUTER JOIN mast_slno ON schal_slno.sl_slno = mast_slno.sl_slno WHERE schal_slno.sch_sl=" & pchsl & "")
    ...
    Private Sub trans_view()
        ...
        Start1()
        Dim ds As DataSet
        ds = get_dataset("SELECT TOP (3) convert(varchar,sch_dt,3) 'Date',STR(mrp,10,2) 'MRP', STR(sale_rt,10,2) 'Rate' FROM schal2 WHERE (it_cd = " & Val(txtitcd.Text) & ") AND (prt_code = " & Val(txtprtcd.Text) & ") ORDER BY sch_dt DESC")
        If ds.Tables(0).Rows.Count <> 0 Then
...
#End Region
    ...
    Private Sub cmnuedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmnuedit.Click
        If dv.RowCount <> 0 Then
            comd = "M"
            btnsave.Text = "Modify"
            Me.Text = "Sales Challan Modify Screen . . ."
    ...
    Private Sub del1(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT sch_no FROM schal1 WHERE sch_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            SQLInsert("DELETE FROM schal1 WHERE sch_no=" & Val(code) & "")
        End If
    End Sub

    Private Sub del2(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT sch_sl,sch_qty,it_cd,bat_cd,free_qty,mul_rt,prd_tp,sch_tp FROM schal2 WHERE sch_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To dsdel.Tables(0).Rows.Count - 1
                ...
                Dim ds2 As DataSet = get_dataset("SELECT sl_slno FROM schal_slno WHERE sch_sl=" & pchsl & "")
                If ds2.Tables(0).Rows.Count <> 0 Then
                    For m As Integer = 0 To ds2.Tables(0).Rows.Count - 1
                        ...
                        SQLInsert("DELETE FROM schal_slno WHERE sl_slno =" & slno & "")
                    Next
                End If
                SQLInsert("DELETE FROM schal2 WHERE sch_sl=" & Val(pchsl) & "")
            Next
        End If
    End Sub
    ...
    Private Sub Set_view()
        ...
        chksetdisc.Checked = False
        chksetcalcrt.Checked = False
        Dim ds As DataSet = get_dataset("SELECT * FROM setting_schal WHERE slno=1")
        If ds.Tables(0).Rows.Count <> 0 Then
