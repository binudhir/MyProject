...
Public Class frmpurret
    Dim comd As String = "E"
    Private allowCoolMove As Boolean = False
    Private myCoolPoint As New Point
#Region "Start Up"
    Private Sub frmpurret_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
    ...
    Private Sub frmpurret_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
        If Asc(e.KeyChar) = 27 Then
            Me.Close()
        End If
    End Sub

    Private Sub frmpurret_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub frmpurret_SizeChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.SizeChanged
...
#Region "Mouse Enter/Leave"
    ...
    Private Sub chksetmep_MouseEnter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chksetvat.MouseEnter, chksetrate.MouseEnter, chksetmrp.MouseEnter, chksetdisc.MouseEnter, chksetcalcrt.MouseEnter, chksetblrnd.MouseEnter, chksetprnt.MouseEnter
        sender.forecolor = Color.Blue
    End Sub

    Private Sub chksetmep_MouseLeave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles chksetvat.MouseLeave, chksetrate.MouseLeave, chksetmrp.MouseLeave, chksetdisc.MouseLeave, chksetcalcrt.MouseLeave, chksetblrnd.MouseLeave, chksetprnt.MouseLeave
        sender.forecolor = Color.Red
    End Sub
#End Region
...
#Region "Button"
    ...
    Private Sub btnsetaply_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsetaply.Click
        Start1()
        SQLInsert("UPDATE setting_pret SET req_mrp=" & IIf(chksetmrp.Checked, 1, 0) & ",req_rate=" & _
                  ...
                  IIf(chksetprnt.Checked, 1, 0) & " WHERE slno=1")
        Close1()
        setting_panel.Visible = False
    End Sub
...
#Region "Data save"
    Private Sub pret_save()
        If comd = "E" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO pret1 (pret_no,pret_dt,prt_code,chal_no,chal_dt,ref_no,ref_dt,way_bno,trk_no,pret_type,pret_amt,way_chr,dis_amt," & _
                ...
                If chksetprnt.Checked = True Then
                    ...
                    If cnfr = vbYes Then
                        Call pret_print(Val(txtscroll.Text))
                    End If
                End If
                Me.clr()
                Close1()
        ...
        ElseIf comd = "M" Then
            ...
            If cnfr = vbYes Then
                ...
                If ds.Tables(0).Rows.Count <> 0 Then
                    SQLInsert("UPDATE pret1 SET pret_dt='" & Format(retdt.Value, "dd/MMM/yyyy") & "',prt_code=" & Val(txtprtcd.Text) & ",chal_no='" & _
                          Trim(txtchno.Text) & "',chal_dt='" & Format(chdt.Value, "dd/MMM/yyyy") & "',ref_no='" & Trim(txtrefno.Text) & "',ref_dt='" & _
                          Format(refdt.Value, "dd/MMM/yyyy") & "',way_bno=" & Val(txtwbno.Text) & ",trk_no='" & Trim(txtvehno.Text) & "',pret_type='" & _
                          vb.Left(cmbrettype.Text, 1) & "',pret_amt=" & Val(txtgrtotal.Text) & ",way_chr='" & Trim(txtwaypfx.Text) & "',dis_amt=" & _
                          ...
                          Val(txtadj.Text) & ",godsl=" & Val(txtgodcd.Text) & " WHERE pret_no =" & Val(txtscroll.Text) & "")
                    Me.dv_save()
        ...
        If dv1.RowCount <> 0 Then
            ...
            For i As Integer = 0 To dv1.RowCount - 1
                ...
                If ds1.Tables(0).Rows.Count <> 0 Then
                    ...
                    End If
                End If
                SQLInsert("INSERT INTO pret2(pret_sl,pret_no,pret_dt,prt_code,it_cd,bat_cd,pur_rt,pret_qty,it_amt,tto_amt,tax_cd,oth_amt,disc_amt," & _
                          "it_tot,stax,entax,mrp,ex_mrp,prd_tp,tr_tp,unt,free_qty,mul_rt,disc_rt1,disc_rt2,disc_amt1,disc_amt2,mrp_amt," & _
        ...
        If dvsl1.RowCount <> 0 Then
            For m As Integer = 0 To dvsl1.RowCount - 1
                ...
                If Val(itcd) = Val(it_cd) Then
                    ...
                    SQLInsert("INSERT INTO pret_slno (sl_slno,it_cd,pret_no,pret_sl) VALUES (" & srlcd & "," & itcd & "," & Val(txtscroll.Text) & "," & Val(pursl) & ")")
                End If
            Next
        End If
    End Sub
    ...
    '                  "dop_stk,drec_stk,diss_stk,dcl_stk,manf_dt,rec_lock,prt_code,pret_dt,inv_no,inv_dt) VALUES (" & Val(txtbatcd.Text) & ",'" & _
...
#Region "Data Retrieve"
    ...
    Private Sub dv_view()
        Dim ds As DataSet = get_dataset("SELECT item.it_name, pret2.*, taxper.txname,batno.bat_no FROM pret2 LEFT OUTER JOIN taxper ON pret2.tax_cd = taxper.taxcd LEFT OUTER JOIN item ON pret2.it_cd = item.it_cd LEFT OUTER JOIN batno ON pret2.bat_cd = batno.bat_cd WHERE pret2.pret_no=" & Val(txtscroll.Text) & "")
        rw = 0
        dv1.Rows.Clear()
        If ds.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To ds.Tables(0).Rows.Count - 1
                ...
                dv1.Rows(rw).Cells(6).Value = Format(ds.Tables(0).Rows(i).Item("pret_qty"), "####0.00")
                ...
                'mrpval = Val(ds.Tables(0).Rows(i).Item("mrp")) * (Val(ds.Tables(0).Rows(i).Item("pret_qty")) + Val(ds.Tables(0).Rows(i).Item("free_qty")))
                ...
                dv1.Rows(rw).Cells(28).Value = ds.Tables(0).Rows(i).Item("pret_sl")
                rw += 1
                'If txtbatcnfr.Text = "S" Then
                pursl = ds.Tables(0).Rows(i).Item("pret_sl")
                Me.srl_view(pursl)
                'End If
            Next
            Me.grandamt(dv1)
    ...
    Private Sub srl_view(ByVal pursl As Integer)
        Dim ds2 As DataSet = get_dataset("SELECT mast_slno.slno, pret_slno.* FROM pret_slno LEFT OUTER JOIN mast_slno ON pret_slno.sl_slno = mast_slno.sl_slno WHERE pret_slno.pret_sl=" & pursl & "")
    ...
    Private Sub dv_edit(ByVal slno As Integer)
        ...
        If dsedit.Tables(0).Rows.Count <> 0 Then
            txtscroll.Text = slno
            retdt.Value = dsedit.Tables(0).Rows(0).Item("pret_dt")
            cmbrettype.SelectedIndex = Val(dsedit.Tables(0).Rows(0).Item("pret_type")) - 1
...
#End Region
    ...
    Private Sub del2(ByVal code As Integer)
        Dim dsdel As DataSet = get_dataset("SELECT pret_sl,pret_qty,it_cd,bat_cd,free_qty,mul_rt,prd_tp,tr_tp  FROM pret2 WHERE pret_no=" & Val(code) & "")
        If dsdel.Tables(0).Rows.Count <> 0 Then
            For i As Integer = 0 To dsdel.Tables(0).Rows.Count - 1
                ...
                Dim ds2 As DataSet = get_dataset("SELECT sl_slno FROM pret_slno WHERE pret_sl=" & pursl & "")
                If ds2.Tables(0).Rows.Count <> 0 Then
                    For m As Integer = 0 To ds2.Tables(0).Rows.Count - 1
                        ...
                        SQLInsert("DELETE FROM pret_slno WHERE sl_slno =" & slno & "")
                    Next
                End If
    ...
    Private Sub Set_view()
        ...
        chksetcodisc.Checked = False
        chksetprnt.Checked = False
        Dim ds As DataSet = get_dataset("SELECT * FROM setting_pret WHERE slno=1")
        If ds.Tables(0).Rows.Count <> 0 Then
