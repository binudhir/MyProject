...
Public Class frmuser
    ...
    Private Sub dv_disp()
        Start1()
        Dim dsedit As DataSet = get_dataset("SELECT ROW_NUMBER() OVER(ORDER BY usr_sl) AS 'Sl.', usr_id AS 'ID', (CASE WHEN usr_tp='A' THEN 'Administrator' WHEN usr_tp='D' THEN 'Director' WHEN usr_tp='M' THEN 'Manager' WHEN usr_tp='O' THEN 'Operator' END) AS 'Type', usr_nm AS 'Name', usr_add1 AS 'Address', usr_ph1 AS 'Contact No.', usr_mail AS 'Mail Id', rec_lock FROM usr ORDER BY usr_sl")
        dv.Columns.Clear()
    ...
    '            dv.Rows(i).Cells(1).Value = dsedit.Tables(0).Rows(rw).Item("usr_id")
    '            If dsedit.Tables(0).Rows(rw).Item("usr_tp") = "A" Then
    '                dv.Rows(i).Cells(2).Value = "Administrator"
    '            ElseIf dsedit.Tables(0).Rows(rw).Item("usr_tp") = "D" Then
    '                dv.Rows(i).Cells(2).Value = "Director"
    '            ElseIf dsedit.Tables(0).Rows(rw).Item("usr_tp") = "M" Then
    '                dv.Rows(i).Cells(2).Value = "Manager"
    '            Else
    '                dv.Rows(i).Cells(2).Value = "User"
    '            End If
    '            dv.Rows(i).Cells(3).Value = dsedit.Tables(0).Rows(rw).Item("usr_nm")
    '            dv.Rows(i).Cells(4).Value = dsedit.Tables(0).Rows(rw).Item("usr_add1")
    '            dv.Rows(i).Cells(5).Value = dsedit.Tables(0).Rows(rw).Item("usr_ph1")
    '            dv.Rows(i).Cells(6).Value = dsedit.Tables(0).Rows(rw).Item("usr_mail")
    '            dv.Rows(i).Cells(7).Value = dsedit.Tables(0).Rows(rw).Item("usr_sl")
    ...
    Private Sub cmbempid_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbempid.GotFocus
        populate(cmbempid, "SELECT usr_id FROM usr WHERE rec_lock='N' ORDER BY usr_id")
    End Sub
    ...
    Private Sub cmbempid_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbempid.Validating
        If comd = "E" Then
            ...
            End If
        Else
            vdating(cmbempid, "SELECT usr_id FROM usr WHERE usr_id='" & Trim(cmbempid.Text) & "' AND rec_lock='N'", "Please Select A Valid Book Name.")
        End If
    End Sub
